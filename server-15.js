...
function broadcastPowers(roomId) {
  ...
  room.members.forEach((memberUser, sid) => {
    ...
    const powersArr = memberUser.power === DEFAULT_ADMIN_POWER.name
      ...
      : room.powers;
    console.log(`[SRV-powers] broadcastPowers → user=${memberUser.id} power=${memberUser.power} powersArr.length=${powersArr.length} names=${powersArr.map(p=>p.name).join(',')}`);
...
function buildPower(user, room) {
  console.log(`[SRV-buildPower] user=${user.id} power="${user.power}" room=${room ? room.id : 'null'} | room.powers=${room ? room.powers.map(p=>p.name).join(',') : 'N/A'}`);
...
function joinRoom(socket, user, room, pwd = '', force = false) {
  ...
  console.log(`[SRV-joinRoom] user=${user.id} power=${user.power} rank=${user.rank} | _joinPowers.length=${_joinPowers.length} names=${_joinPowers.map(p=>p.name).join(',')} | buildPower.cp=${_powerForUser.cp} buildPower.owner=${_powerForUser.owner}`);
  const rcdPayload = [
  ...
  socket.on('rc2', ({ token, n } = {}) => {
    ...
    const _powersRoom = rooms.get(user.roomid) || [...rooms.values()][0];
    {
      ...
      const _powersForUser = user.power === DEFAULT_ADMIN_POWER.name
        ...
        : _bp;
      console.log(`[SRV-rc2] powers → user=${user.id} power=${user.power} | _powersForUser.length=${_powersForUser.length} names=${_powersForUser.map(p=>p.name).join(',')}`);
