...
app.get('/', (req, res) => {
  ...
  if (contentColor) {
    // ▲ إضافة: .pmsgc (إعلانات عادية/تلقائية) و.ppmsgc (إعلانات السوابر)
    //   و.hmsg (رسائل النظام: غادر الغرفة/تم طرده/انتقل/حظر/خروج/انقطاع)
    //   لها خلفية ثابتة بـ!important معرَّفة مباشرة داخل index.html نفسه
    //   (pmsgc: أزرق شفاف خفيف، ppmsgc: بنفسجي فاتح، hmsg: لون linen) —
    //   فتتجاهل أي لون محتوى مخصص تماماً حتى الآن. قاعدتنا هنا تُحقن بعد
    //   الأصلية بنفس الملف (لاحقة بالترتيب) وبنفس القوة (!important)، فتفوز
    //   بالتعادل وتُطبَّق فعلياً.
