...
app.get('/', (req, res) => {
  ...
  //
  //   • لون القالب (bg)      → .bg فيما عدا body وأزرار شريط المايكات ولوحة
  //     الدخول بالكامل (.center-block.dad وكل ما بداخلها) — ▲ تعديل (طلب
  //     صريح): لوحة الدخول كانت تُلوَّن كوحدة واحدة (الحاوية + الشريط العلوي
  //     + شريط متصل/عدد الأعضاء كلها بنفس اللون دفعة واحدة)، فاستُثنيت
  //     بالكامل من هذه القاعدة العامة، وبدلاً منها يُلوَّن منها تحديداً فقط
  //     عنصران، كل منهما بقاعدته الخاصة (أدناه): الشريط العلوي (رابط
  //     الشعار+الاسم+زر التحديث) وشريط الحالة (متصل/عدد الأعضاء) — دون أن
  //     يمتد اللون لما بداخلهما (صورة الشعار وزر التحديث في الأول، نص متصل
  //     ورقم الأعضاء في الثاني تبقى بلونها الأصلي كما كانت). بقية عناصر .bg
  //     خارج لوحة الدخول (المايك، اللوحة الجانبية، الغرفة النشطة...) لم
  //     تُمَس، تعمل بالضبط كسابقاً.
  ...
  if (templateColor) {
    colorRules.push(`.bg:not(body):not(#mic *):not(.center-block.dad):not(.center-block.dad *){background-color:${templateColor} !important;}`);
    // ▲ إضافة: عنصرا لوحة الدخول المحدَّدين تحديداً — كل قاعدة تستهدف
    //   العنصر نفسه فقط بكامل كلاساته (لا تطابق أي عنصر آخر بالصفحة)، فلا
    //   تمسّ أبناءه: الشريط لا يمسّ <img> شعار الموقع ولا زر التحديث
    //   (fa-refresh) بداخله، وشريط الحالة لا يمسّ label#loginstat ("متصل")
    //   ولا span#s1 (عدد الأعضاء) — لأن background-color خاصية لا تُورَّث،
    //   وهذان الابنان لا يحملان كلاس bg أصلاً.
    colorRules.push(`.label.bg.d-flex.fl{background-color:${templateColor} !important;}`);
    colorRules.push(`.bg.mini.fl{background-color:${templateColor} !important;}`);
  }
  if (contentColor) {
    // ▲ إصلاح (مستند "المرحلة الثالثة" — البند 1): .pmsgc/.ppmsgc/.hmsg
    //   حُذفت من هذه القائمة لأن index.html نفسه يعطيها خلفية ثابتة مقصودة
    //   بـ!important (لتمييزها بصرياً): .pmsgc=rgba(0,77,255,.08)،
    //   .ppmsgc=#f1f1ff، .hmsg=linen — يطابق "الألوان الخاصة بالإعلانات
    //   تبقى كما هي إذا كانت محددة في القالب".
    colorRules.push(`.d2,#rooms,#users,#lonline,.uzr,.room{background-color:${contentColor} !important;}`);
    // ▲ محاولة ثانية بأسلوب مختلف تماماً (طلب صريح لاحق: "pmsgc/ppmsgc اقصد
    //   ذول" — أي أن اندماجهما بلون واحد لا يزال قائماً): appraad2.js يضع
    //   class="uzr ... pmsgc" على نفس عنصر صف الإعلان، فقاعدة .uzr أعلاه
    //   وحدها كافية لتلوين خلفيته بـcontentColor أيضاً رغم حذف .pmsgc من
    //   قائمتها، فتظهر كل الإعلانات (وrسائل النظام) بنفس لون المحتوى بدل
    //   ألوانها المميِّزة الثلاثة — وهذا بالضبط "دمج أنواعها".
    //   المحاولة الأولى استثنت .uzr بـ:not(.pmsgc):not(.ppmsgc):not(.hmsg)،
    //   فأُلغيت بطلب صريح (أفاد المستخدم بإفساد أبعاد الشاشة؛ لم يتّضح لي
    //   السبب التقني، فقاعدة :not() على background-color فقط لا تمسّ
    //   layout إطلاقاً — ربما كان السبب مكاناً آخر لم يُحدَّد). هذه المرة:
    //   بدل استثناء .uzr، أُضيف قواعد أقوى تحديداً (خصوصية CSS أعلى:
    //   class.class = 0-2-0 يتغلب دائماً على class واحدة = 0-1-0 عند تساوي
    //   !important، بصرف النظر عن ترتيب الظهور في المستند) تكرّر ألوان
    //   index.html الثابتة لنفس هذه الكلاسات الثلاث تحديداً. إن ظهرت نفس
    //   مشكلة "أبعاد الشاشة" مجدداً بعد هذا التعديل تحديداً فهي مؤكَّدة من
    //   نظام الألوان لا من مكان آخر — أخبرني فوراً لأتراجع.
    colorRules.push(`.uzr.pmsgc{background-color:rgba(0,77,255,.08) !important;}.uzr.ppmsgc{background-color:#f1f1ff !important;}.uzr.hmsg{background-color:linen !important;}`);
  }
  if (buttonsColor) {
