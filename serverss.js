...
app.get('/', (req, res) => {
  ...
  if (contentColor) {
    // ▲ إعادة تطبيق (مستند "المرحلة الثالثة" الجديد يُلغي طلب التراجع
    //   السابق صراحة، بند أول: "جميع الرسائل التي تستخدم لون المحتوى تتبع
    //   إعداد لون المحتوى مباشرة" — بما فيها صراحة رسائل النظام والإعلانات
    //   العامة/الخاصة). .pmsgc/.ppmsgc/.hmsg لها خلفية ثابتة بـ!important
    //   داخل index.html نفسه، فتُدرَج هنا بنفس القوة لتفوز بالتعادل (قاعدتنا
    //   لاحقة بترتيب الحقن، فتربح عند تساوي الخصوصية والقوة).
    colorRules.push(`.d2,#rooms,#users,#lonline,.uzr,.room,.pmsgc,.ppmsgc,.hmsg{background-color:${contentColor} !important;}`);
  }
  if (buttonsColor) {
...
class User {
  ...
  pub() {
    return {
      ...
      // ▲ إصلاح: appraad2.js يبني "imgs/s"+stat+".png" لأيقونة الحالة
      //   (.ustat) — هذا الحقل لم يكن موجوداً إطلاقاً بكل الملف، فتظهر
      //   "sundefined.png" دائماً. القيمة الوحيدة المؤكَّدة من طلب صريح:
      //   2 = قفل الخاص (nopm) مفعّل. 1 = الحالة الافتراضية خلاف ذلك (لا رقم
      //   آخر محدَّد بأي طلب حتى الآن لحالات أخرى — قابل للتوسعة لاحقاً).
      stat:     this.nopm ? 2 : 1
    };
  }
}
  ...
  socket.on('rc2', ({ token, n } = {}) => {
    ...
    if (found) {
      ...
      if (originalSocket && originalSocket.id !== socket.id) {
        // ▲ إصلاح: 'kicked_login' ليست من ضمن حالات login.msg الست التي
        //   يتعرّف عليها appraad2.js (ok/noname/badname/usedname/badpass/wrong/reg)،
        //   فلا تظهر أي رسالة للجلسة القديمة. نُضيف 'not' كتنبيه مفهوم فعلياً
        //   دون حذف الإرسال الأصلي تحسباً لأي استخدام آخر له.
        send(originalSocket, 'not', { user: 'srv', msg: 'تم تسجيل الدخول من جهاز/نافذة أخرى' });
  ...
  switch (cmd) {
    ...
    case 'login': {
      ...
      if (existingUser && existingUser.socketId !== socket.id) {
        ...
        if (oldSocket) {
          send(oldSocket, 'not', { user: 'srv', msg: 'تم تسجيل الدخول من جهاز/نافذة أخرى' });
    ...
    case 'call': {
      ...
      if (!target) {
        // ▲ إصلاح: كان يفشل بصمت بلا أي رد للمتصل لو الطرف غير موجود إطلاقاً
        if (callT === 'call') send(socket, 'not', { user: 'srv', msg: 'هذا العضو غير متصل حالياً' });
        return;
      }
...
//   المطلوب: "Linux | x00 | vr.07n.n9.cy | ...".
    ...
    // ملاحظة: أوامر 'gift' و'not' و'ubnr' كانت موجودة هنا كمستوى أعلى مباشر،
    // لكن appraad2.js لا يرسلها إلا عبر action:{cmd:'gift'|'not', id, ...}
    // (تُعالَج بدالة dispatchCP)، أو لا يرسل 'ubnr' إطلاقاً — فحُذفت لعدم
    // وصول appraad2.js إليها أبداً، وبقيت النسخ الفعّالة فقط.
    ...
    // ════════════════════════════════════════════════════════════════════════
    case 'uh': {
      // ▲ إصلاح: كان يفشل بصمت بلا أي رد لو الصلاحية مرفوضة
      if (!adminOk && !hasPower(user, room, 'history')) {
        send(socket, 'not', { user: 'srv', msg: 'ليس لديك صلاحية عرض سجل الأعضاء' });
        return;
      }
    ...
    case 'bnr': {
      ...
      // ▲ إصلاح: لم يكن يصل أي تأكيد للمشرف نفسه بعد تنفيذ العملية
      send(socket, 'ok', null);
      break;
    }
    ...
    // ════════════════════════════════════════════════════════════════════════
    case 'setLikes': {
      // ▲ إصلاح: صلاحية "setLikes" مُعرَّفة أصلاً بنظام الصلاحيات ولم تكن
      //   تُفحَص هنا (adminOk فقط) — كانت مُطبَّقة فقط على نسخة 'likes'
      //   المكررة داخل dispatchCP التي لا يصلها appraad2.js إطلاقاً.
    ...
    // ملاحظة: 'udelpic' كانت هنا كنسخة مكررة من action:{cmd:'delpic'}
    // (تُعالَج بدالة dispatchCP)، لكنها لا تفحص تسلسل الرتب كما تفعل نسخة
    // delpic، وappraad2.js لا يرسلها أصلاً — فحُذفت.
...
function dispatchCP(socket, user, room, data, adminOk, modOk) {
  ...
  switch (cmd) {
    ...
    // ملاحظة: 'likes' كانت هنا كنسخة مكررة من 'setLikes' (تُعالَج بدالة
    // dispatch الرئيسية)، وappraad2.js لا يرسلها عبر cp إطلاقاً — حُذفت بعد
    // نقل تحسين فحص صلاحيتها إلى setLikes.
    ...
    // ملاحظة: 'unick' كانت هنا كنسخة مكررة (بصلاحية adminOk فقط، بلا
    // التفريق بين mynick/unick) من النسخة الموثّقة على مستوى dispatch
    // الرئيسية، وappraad2.js يرسلها مباشرة (غير مغلّفة بـcp/action) — حُذفت.

    // ── سجل تسجيل الدخول  { q, i }
    // appraad2.js case 'cp_logins' → يقرأ: u, t, ip, fp, power, rep, lastseen, regdate
    // ▲ إضافة (البند الثالث): استرجاع سجل تسجيل الدخول عبر القناة المصادَق
    //   عليها بالفعل (adminOk، نفس نمط بقية أوامر CP) بدل الاعتماد حصراً على
    //   REST الخارجية المحمية بـADMIN_KEY — الأحدث أولاً.
    case 'loginlog': {
      if (!adminOk) return;
      send(socket, 'cp_loginlog', loginLog.slice().reverse());
      break;
    }
    ...
    case 'shrtdel': {
      ...
      // ▲ إصلاح: لم تكن تُعيد القائمة المحدَّثة بعد الحذف (خلافاً لـ shrt/shrtadd)
      send(socket, 'cp_shrt', Object.entries(global.shortcuts).map(([n, v]) => ({ name: n, value: v })));
      break;
    }
    ...
    case 'room': {
      ...
      try {
        ...
        // ▲ إصلاح: لم يكن يصل أي تأكيد نجاح لمن رفع الملف (خصوصاً pic/room اللتين لا تبثّان ev)
        send(socket, 'ok', null);
      } catch (e) {
        console.error(`[${cmd}] فشل النسخ:`, e.message);
        // ▲ إصلاح: كان الفشل يُسجَّل بالسيرفر فقط دون أي إعلام للمشرف الذي رفع الملف
        send(socket, 'not', { user: 'srv', msg: 'فشل حفظ الملف، حاول مرة أخرى' });
      }
      break;
    }
...
app.delete('/api/rooms/:id', (req, res) => {
  ...
  r.members.forEach((_, sid) => {
    const s = io.sockets.sockets.get(sid);
    // ▲ إصلاح: كان يرسل 'kick' خاماً الذي لا يملك appraad2.js أي معالج له
    //   (نفس علة مسار الطرد الفردي أعلاه)، فيفشل بصمت بلا أي رسالة أو قطع فعلي.
    if (s) {
      send(s, 'not', { user: 'srv', msg: 'تم حذف الغرفة' });
      send(s, 'close', {});
      setTimeout(() => { try { s.disconnect(true); } catch (_) {} }, 1200);
    }
  });
