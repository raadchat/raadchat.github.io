...
app.use('/sico', express.static(UPLOAD_DIR));
app.use('/dro3', express.static(UPLOAD_DIR));
app.use('/emo',  express.static(UPLOAD_DIR));
...
app.get('/', (req, res) => {
  ...
  // ▲ إعادة بناء كاملة لنظام حقن الألوان بعد توضيح دقيق من المستخدم مع لقطتي
  //   شاشة، وتحققتُ من كل عنصر في index.html حرفياً قبل الحقن:
  //   - "لون القالب" (bg): الأشرطة (شريط اسم الموقع، شريط "متصل"/عدد
  //     الزوار، شريط المايكات، حاوية الأزرار السفلية) — كلها تحمل صنف .bg
  //     الحقيقي (وليس فقط body نفسه كما كان الحقن السابق يفترض خطأً).
  //   - "لون المحتوى" (background): المساحات البيضاء (قوائم الرسائل/الغرف/
  //     الأعضاء) — وجدتُ قاعدة CSS حقيقية جاهزة تستهدفها تحديداً:
  //     ".d2,#rooms,#users{background-color:#fafafa!important;}" (ويطابق
  //     تماماً القيمة الافتراضية fafafa المحفوظة أصلاً بإعدادات النطاق) —
  //     إضافةً لعناصر أخرى بنفس اللون كنمط ثابت (#lonline, .uzr, .room).
  //   - "لون الأزرار" (buttons): قاعدة .btn-primary/.label-primary/
  //     .bg-primary الموجودة أصلاً (تُستخدم لكل أزرار الدخول والأزرار
  //     السفلية كما وصفتَ بالضبط).
  //   الحقن الآن كتلة <style> حقيقية (بدل استبدال قيمة سمة واحدة هشة) —
  //   أكثر أماناً (لا يمكن أن يُفسد أي سمة أخرى مجاورة) وأدق (يغطي كل مكان
  //   يظهر فيه كل لون فعلياً، لا موضعاً واحداً فقط).
  const colorRules = [];
  const hexOf = (v) => {
  ...
  if (templateColor) {
    // ▲ تصحيح: عنصر body (حتى لو يحمل كلاس bg) يُستثنى نهائياً من نظام
    //   الألوان بطلب صريح — يبقى بمظهره الأصلي تماماً كما هو في index.html
    //   دون أي ارتباط بلون القالب، بخلاف بقية عناصر .bg (شريط الاسم/المتصل/
    //   المتواجدين/خلفية شريط المايكات) التي تستمر بالتلوّن بلون القالب
    //   كالمعتاد. ‎:not(body)‎ يمنع أي تطبيق حتى لو أُضيف كلاس bg لعنصر
    //   body مستقبلاً، و‎:not(#mic *)‎ يبقي استثناء أزرار المايك كما هو.
    colorRules.push(`.bg:not(body):not(#mic *){background-color:${templateColor} !important;}`);
  }
  if (contentColor) {
    // ▲ إضافة: بجانب اللون، نضمن أن هذه العناصر (وتحديداً .room و.uzr، صفوف
    //   الغرف/الأعضاء) تملأ عرض حاويتها بالكامل فعلياً (width:100% +
    //   box-sizing:border-box يمنع تجاوز العرض بسبب أي padding داخلي) — لا
    //   تكفي إعادة تلوينها فقط، لأن أي فراغ جانبي كان موجوداً أصلاً لكنه
    //   غير مرئي (نفس لون الخلفية من حوله) يصبح ظاهراً بمجرد فرض لون مختلف
    //   عليها تحديداً هي (لا على العنصر المحيط/الخلفية العامة).
    colorRules.push(`.d2,#rooms,#users,#lonline,.uzr,.room{background-color:${contentColor} !important;width:100%!important;box-sizing:border-box!important;}`);
  }
  if (buttonsColor) {
    // ▲ تصحيح: كلاس bg-* (مثل bg-primary) يجب أن يبقى مستقلاً تماماً عن كل
    //   نظام الألوان — طلب صريح: "لا يتبع لون الأزرار... لا يتغير لونه
    //   تلقائياً عند تغيير أي لون في النظام". أُزيل .bg-primary نهائياً من
    //   هذه القاعدة (بقي .label-primary/.btn-primary فقط، وهي أزرار حقيقية
    //   لا علاقة لها بكلاس bg-*).
  ...
  // ▲ إضافة: قاعدة أمان دائمة (تُحقن دوماً بغض النظر عن وجود ألوان مخصصة)
  //   تمنع تمدد عرض/ارتفاع الصفحة كلياً بسبب نص طويل بلا فواصل (مثل الكلمات
  //   الدلالية) — overflow-wrap يجبر أي نص طويل متصل على الالتفاف بدل دفع
  //   الصفحة للاتساع أفقياً، ومربعا الكلمات الدلالية تحديداً تُقيَّد بعرضهما
  //   الأصلي 260px بصرامة (max-width!important) حتى لو حاول المتصفح توسيعهما
  //   تلقائياً لمحتوى طويل جداً بلا مسافات.
  const layoutSafetyCss = '<style>html,body{max-width:100vw;overflow-x:hidden;}'
    + '#sett_keywords,#domain_keywords{max-width:260px!important;overflow-wrap:break-word;word-break:break-all;}'
    // ▲ إضافة: الحاوية الرئيسية (.center-block.dad) مقيَّدة بعرض ثابت
    //   max-width:394px مباشرة بسطر النمط بـ index.html — مع تصحيح viewport
    //   أعلاه (device-width بدل 400 ثابتة)، هذا القيد الثابت هو الفراغ
    //   الرمادي المتبقي: أي جهاز أعرض من 394px منطقياً يظهر عنده فراغ
    //   بالفرق. نرفعه هنا ليملأ العرض الفعلي كاملاً بدل حد أقصى ثابت.
  ...
  if (colorRules.length) {
    ...
    html = html.includes(closeHeadTag)
      ? html.replace(closeHeadTag, styleTag + closeHeadTag)
      : styleTag + html;
  }
...
(function () {
  ...
  function pollRoomBans() {
    if (typeof myroom === 'undefined' || !myroom) return;
    if (!$('#ops').is(':visible')) return;
    fetch('/api/roombans?room=' + encodeURIComponent(myroom))
      .then(function (r) { return r.json(); })
      .then(renderRoomBans)
      .catch(function () {});
  }
  setInterval(pollRoomBans, 4000);
  setTimeout(pollRoomBans, 1500);
})();
</script>`;
...
function dispatch(socket, user, cmd, data) {
  ...
  switch (cmd) {
    ...
    case 'file': {
      ...
      const payloadToRecipient = buildMsg(user, '', { pm: user.id,   bid, uid: user.id });
      const payloadToSender    = buildMsg(user, '', { pm: target.id, bid, uid: user.id });
...
function dispatchCP(socket, user, room, data, adminOk, modOk) {
  ...
  switch (cmd) {
    ...
    case 'addico': {
      ...
      logAction(user, 'addico', room.name, `${tar}: ${data.pid}`);
      ...
      toRoom(room.id, 'ico+', tar + '/' + data.pid);
      break;
    }
    ...
    case 'sitesave': {
      ...
      if (hasSharedToggle) {
        rooms.forEach(r => {
          ...
          toRoom(r.id, 'settings', r.settings);
        });
      }
      send(socket, 'ok', null);
      break;
