...
app.get('/', (req, res) => {
  ...
  if (s.bg) {
    // body.bg يحمل بالفعل background-color ثابتة داخل style المضمّن — نستبدلها مباشرة
    const bgColor = String(s.bg).startsWith('#') ? s.bg : '#' + s.bg;
    html = html.replace(/(<body class="bg"[^>]*style="background-color:)#[0-9a-fA-F]{3,6}(;)/, `$1${bgColor}$2`);
  }
  if (s.script) {
  ...
  switch (cmd) {
    ...
    case 'prv1': {
      ...
      try {
        ...
        const destName = cmd === 'favicon' ? 'favicon.ico' : 'prv1.png';
        fs.copyFileSync(srcPath, path.join(__dirname, 'public', destName));
        logAction(user, cmd, destName);
        // إعلام كل المتصلين لتحديث الأيقونة/الصورة في المتصفح فوراً (كسر التخزين المؤقت بـ query string)
        toAll('ev', {
          data: cmd === 'favicon'
            ? `try{$("link[rel=icon]").attr("href","favicon.ico?"+Date.now());}catch(e){}`
            : `try{$("img[src^='prv1.png']").attr("src","prv1.png?"+Date.now());}catch(e){}`
        });
      } catch (e) {
        console.error('[favicon/prv1] فشل النسخ:', e.message);
      }
      break;
    }
