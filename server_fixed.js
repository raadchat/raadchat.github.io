...
app.get('/', (req, res) => {
  ...
  if (s.bg) {
    // ▲ إصلاح: تحقق صارم من كون القيمة لون hex صالح (3 أو 6 خانات، مع أو
    //   بدون #) قبل الحقن — أي قيمة غير صالحة (مثلاً تحتوي فاصلة منقوطة أو
    //   نصاً حراً) كانت قد تُفسد سمة style بالكامل (بما فيها خصائص القياس
    //   المجاورة height/margin/padding) فتُسبب مشاكل بصرية بحجم الصفحة —
    //   الآن يُتجاهل الحقن كلياً إن كانت القيمة غير صالحة، بدل حقنها كما هي.
    const bgRaw = String(s.bg).trim().replace(/^#/, '');
    if (/^[0-9a-fA-F]{6}$|^[0-9a-fA-F]{3}$/.test(bgRaw)) {
      html = html.replace(/(<body class="bg"[^>]*style="background-color:)#[0-9a-fA-F]{3,6}(;)/, `$1#${bgRaw}$2`);
    }
  }
  if (s.script) {
    // سكربت المبرمج المخصص — يُحقن قبل نهاية body (بعد appraad2.js) كما في نمط /cp
    const closeBodyTag = '</body>';
    const customScript = '<script>' + s.script + '</script>';
