...
app.get('/', (req, res) => {
  ...
  if (templateColor) {
    // ▲ إضافة: استثناء شريط المايكات بالكامل (#mic والحاوية نفسها + زر
    //   "كتم الكل" #muteall + كل سلوتات المايك #mic0..#mic4 بداخلها) من لون
    //   القالب — طلب صريح: تحتفظ هذه العناصر بألوانها الأصلية دائماً ولا
    //   تتأثر بتغيير لون القالب أو الأزرار إطلاقاً. ‎:not(#mic *)‎ يستثني كل
    //   عنصر داخل #mic (مدعوم بكل المتصفحات الحديثة)، فيبقى زر #muteall
    //   يعمل بمنطقه الأصلي في appraad2.js (أخضر=نشط، شفاف=مكتوم) دون تدخل.
    colorRules.push(`body.bg,.bg:not(#mic):not(#mic *){background-color:${templateColor} !important;}`);
  }
  if (contentColor) {
    colorRules.push(`.d2,#rooms,#users,#lonline,.uzr,.room{background-color:${contentColor} !important;}`);
  }
  if (buttonsColor) {
    colorRules.push(`.label-primary,.btn-primary,.bg-primary,.label-primary:hover,.btn-primary:hover,.btn-primary:focus{background-color:${buttonsColor} !important;background-image:none !important;}`);
  }
