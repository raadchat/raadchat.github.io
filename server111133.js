...
app.get('/', (req, res) => {
  ...
  if (templateColor) {
    // ▲ تصحيح: الحاوية #mic نفسها (شريط الخلفية خلف أزرار المايك) يجب أن
    //   تبقى تابعة للون القالب (طلب صريح: "الشريط الخلفي فقط هو الذي يتبع
    //   لون القالب") — الاستثناء ‎:not(#mic *)‎ وحده يستثني ما بداخل #mic
    //   (الأزرار: #muteall + سلوتات #mic0..#mic4) دون استثناء #mic ذاتها،
    //   فتتلون خلفية الشريط بلون القالب بينما تبقى الأزرار فوقه بلونها
    //   الأصلي كما هي (لا تدخل appraad2.js إطلاقاً في تغييرها اللحظي لهذه
    //   الأزرار، مثل تبديل #muteall بين أخضر/شفاف حسب حالة الكتم).
    colorRules.push(`body.bg,.bg:not(#mic *){background-color:${templateColor} !important;}`);
  }
  if (contentColor) {
