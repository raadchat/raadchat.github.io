...
app.get('/', (req, res) => {
  ...
  if (contentColor) {
    // ▲ تراجع عن إصلاح سابق بطلب صريح: .pmsgc/.ppmsgc/.hmsg تبقى بألوانها
    //   الأصلية الثابتة المعرَّفة في index.html نفسه (أزرق شفاف خفيف/بنفسجي
    //   فاتح/linen على الترتيب) ولا تتبع لون المحتوى إطلاقاً — لا نلمسها هنا
    //   نهائياً فتُطبَّق قاعدتها الأصلية بلا أي تدخل من حقننا.
...
function dispatch(socket, user, cmd, data) {
  ...
  switch (cmd) {
    ...
    case 'online': {
      ...
      //   الصفحة. الآن تُقرأ من global.siteSettings الحقيقية (نفس ما يكتبه
      //   sitesave بالضبط)، بنفس القيم الافتراضية السابقة فقط عند غيابها.
      {
        const gs = global.siteSettings || {};
        send(socket, 'settings', {
          mlikes:  gs.mlikes  ?? true,
          bclikes: gs.bclikes ?? true,
          mreply:  gs.mreply  ?? false,
          bcreply: gs.bcreply ?? false,
          calls:   gs.calls   ?? false
        });
      }
    ...
    case 'msg': {
      ...
      if (replyMi && !room.settings.mreply) replyMi = null;
      ...
      //   عند الرد على نفس رسالتك.
      if (replyMi) {
        const original = recentMessages.get(replyMi);
        if (original && original.uid !== user.id) {
          const originalAuthor = byUID(original.uid);
          const oaSocket = originalAuthor ? io.sockets.sockets.get(originalAuthor.socketId) : null;
          if (oaSocket) {
            send(oaSocket, 'not', {
              user: user.id, uid: user.id, topic: user.topic, pic: user.pic,
              msg: `قام <b>${user.topic}</b> بالرد على رسالتك`, t: Date.now(), mi: newMi
            });
          }
        }
      }
    ...
    case 'bc': {
      ...
      if (replyBid) {
        ...
        if (originalPost && originalPost.uid !== user.id) {
          const originalAuthor = byUID(originalPost.uid);
          const oaSocket = originalAuthor ? io.sockets.sockets.get(originalAuthor.socketId) : null;
          if (oaSocket) {
            send(oaSocket, 'not', {
              user: user.id, uid: user.id, topic: user.topic, pic: user.pic,
              msg: `قام <b>${user.topic}</b> بالرد على منشورك بالحائط`, t: Date.now(), bid: newBid
            });
          }
        }
      }
      ...
      if (wallMsg) {
        ...
        if (sender && sender.id !== user.id) {
          ...
          if (sSocket) {
            send(sSocket, 'not', {
              user: user.id, uid: user.id, topic: user.topic, pic: user.pic,
              msg: `أعجب <b>${user.topic}</b> بمنشورك بالحائط`, t: Date.now(), bid
            });
          }
        }
      }
    ...
    case 'likem': {
      ...
      if (msgEntry) {
        ...
        if (msgEntry.uid !== user.id) {
          ...
          const aSocket = author ? io.sockets.sockets.get(author.socketId) : null;
          if (aSocket) {
            send(aSocket, 'not', {
              user: user.id, uid: user.id, topic: user.topic, pic: user.pic,
              msg: `أعجب <b>${user.topic}</b> برسالتك`, t: Date.now(), mi: bid
            });
          }
        }
      }
  ...
  switch (cmd) {
    ...
    case 'like': {
      ...
      if (tSocket) {
        send(tSocket, 'not', {
          user: user.id, uid: user.id, topic: user.topic, pic: user.pic,
          msg: `قام <b>${user.topic}</b> بالإعجاب بك`, t: Date.now()
        });
      }
      break;
    }
