...
function leaveRoom(socket, user, reason = 'leave') {
  ...
  // ─── رسالة نظام: سبب المغادرة (تُبثّ لبقية أعضاء الغرفة فقط) ──────────────
  // ▲ إصلاح: كانت هذه الرسالة (مكتوبة بصيغة الغائب: "هذا المستخدم تم
  //   طرده"، مُعدَّة أصلاً لإعلام *بقية* أعضاء الغرفة) تُرسَل أيضاً للمستخدم
  //   المتأثر نفسه في كل الحالات (طرد/حظر/مغادرة عادية/تنقل بين الغرف) عبر
  //   send(socket,'msg',sysMsg) — فكان يستقبل 3 إشارات متداخلة لنفس الحدث
  //   دفعة واحدة: رسالة دردشة بصيغة غريبة عن نفسه + إشعار not واضح مخصص له
  //   (بالنسبة للطرد/الحظر) + إغلاق الجلسة (close) — تشويش وازدواجية واضحة.
  //   الحل: هذه الرسالة تُبثّ الآن *فقط* لبقية أعضاء الغرفة (toRoom)، بينما
  //   الشخص المتأثر نفسه يعتمد على القناة المناسبة له تحديداً: إشعار 'not'
  //   صريح في حالات الطرد/الحظر (تُرسَل من مواضع الاستدعاء نفسها)، أو تحديث
  //   الواجهة التلقائي عبر 'ur' (تعطيل مربع الكتابة) في حالة المغادرة/التنقل
  //   العادي دون الحاجة لأي رسالة دردشة إضافية.
  const sysTexts = {
  ...
  // تُرسل لأعضاء الغرفة فقط (المستخدم أُزيل من members أعلاه فلن يستقبلها عبر toRoom)
