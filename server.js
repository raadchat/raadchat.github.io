 ...
 *
 * الحل: أدرج DEFAULT_ADMIN_POWER في المصفوفة المُرسَلة للأدمن فقط
...
function broadcastPowers(roomId) {
  ...
  room.members.forEach((memberUser, sid) => {
    const s = io.sockets.sockets.get(sid);
    if (!s) return;
    // الأدمن: أضف DEFAULT_ADMIN_POWER إذا لم يكن موجوداً مسبقاً في المصفوفة
    const powersArr = memberUser.rank >= 9000
...
function joinRoom(socket, user, room, pwd = '', force = false) {
  ...
  // أدرج DEFAULT_ADMIN_POWER في مصفوفة الصلاحيات للأدمن
  // حتى يجدها _0x3e8a07 ويبقى _0x41c3fc.cp=true ولا تُغلق نوافذ CP عبر _0x515435()
  const _joinPowers = user.rank >= 9000
    ? (room.powers.some(p => p.name === DEFAULT_ADMIN_POWER.name)
  ...
  const rcdPayload = [
    ...
    ['bclist',   globalWall],   // الحائط عالمي مشترك بين جميع الغرف
...
io.on('connection', socket => {
  ...
  socket.on('rc2', ({ token, n } = {}) => {
    ...
    const _powersRoom = rooms.get(user.roomid) || [...rooms.values()][0];
    {
      ...
      const _powersForUser = user.rank >= 9000
    ...
    if (user.roomid) {
      ...
      if (room) {
        ...
        const _rcdPowers = user.rank >= 9000
          ? (room.powers.some(p => p.name === DEFAULT_ADMIN_POWER.name)
              ? room.powers
              : [...room.powers, DEFAULT_ADMIN_POWER])
          : room.powers;
