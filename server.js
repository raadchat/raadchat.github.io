...
app.get('/', (req, res) => {
  ...
  if (contentColor) {
    ...
    // ▲ تراجع صريح بطلب المستخدم: أُضيف هنا مؤقتاً استثناء
    //   `.uzr:not(.pmsgc):not(.ppmsgc):not(.hmsg)` لأن appraad2.js يضع
    //   class="uzr ... pmsgc" على نفس عنصر صف الإعلان (.uzr وحدها كافية
    //   للمطابقة رغم حذف .pmsgc أعلاه). أُلغي هذا الاستثناء بطلب صريح — أفاد
    //   المستخدم أنه أفسد أبعاد الشاشة بعد التجربة الفعلية؛ لم يتّضح لي
    //   تقنياً كيف (الإضافة كانت :not() على قاعدة background-color فقط، لا
    //   شيء يمس width/height/layout)، فقد تكون العلّة في مكان آخر — لكن
    //   التراجع كان الطلب الصريح فنُفِّذ فوراً. المشكلة الأصلية (خلفية
    //   .pmsgc/.ppmsgc/.hmsg لا تزال تتبع لون المحتوى رغم حذفها من هنا،
    //   بسبب .uzr المشتركة) تبقى غير محلولة حالياً — بانتظار توجيه لاحق.
