...
/** هل لدى المستخدم صلاحية محددة في power definition؟ */
function hasPower(user, room, perm) {
  if (user.rank >= 9000) return true;
  if (!room || !user.power) return false;
  const def = room.powers.find(p => p.name === user.power);
  return def ? !!def[perm] : false;
}
...
function buildPower(user, room) {
  ...
  // أدمن النظام (rank ≥ 9000) → صلاحية كاملة مباشرة
  if (user.rank >= 9000) {
    return {
      ...DEFAULT_ADMIN_POWER,
      name:      user.power || DEFAULT_ADMIN_POWER.name,
      rank:      user.rank,
      roomowner: !!(room && room.owner === user.lid) || true,
      owner:     true
    };
  }

  // ابحث عن تعريف الصلاحية في قائمة الغرفة
...
function seedAdmin() {
  ...
  accounts.set(nameKey, {
    ...
    rank:    9999,          // أعلى رتبة ← isAdmin() يتحقق rank >= 9000
    power:   'الادارة',
...
function joinRoom(socket, user, room, pwd = '', force = false) {
  ...
  const rcdPayload = [
    ...
    ['bclist',   room.wall],
    ['ulist',    [...room.members.values()].map(u => u.pub())],
  ...
  // ─── ③ إبلاغ بقية الأعضاء بالدخول (بعد rcd) ─────────────────────────────
  // u+ يضيف العضو للقائمة، ur يضع عليه كلاس inroom عند الأعضاء الآخرين
  toRoom(room.id, 'u+', user.pub(), socket.id);
  toRoom(room.id, 'ur', [user.id, room.id], socket.id);
...
function leaveRoom(socket, user, reason = 'leave') {
  ...
  // تُرسل قبل u- حتى يكون المستخدم لا يزال في allUsersList عند العملاء الآخرين
  toRoom(prevRoomId, 'msg', buildSysMsg(user, sysText, { mi: makeBid(), 'class': 'hmsg' }));

  // u- يزيل العضو من قائمة الأعضاء لدى بقية الغرفة
  toRoom(prevRoomId, 'u-', user.id);
  // ur[id,null] يُبلّغ العميل المغادر بمسح myroom ويُحدّث حالة الغرفة لجميع العملاء
  toAll('ur', [user.id, null]);
  broadcastRoomUpdate(room);
}
...
io.on('connection', socket => {
  ...
  socket.on('rc2', ({ token, n } = {}) => {
    ...
    // البحث عن الجلسة القديمة بالتوكن — قد تكون من سوكت سابق منقطع
    // أو محفوظة في الـ LocalStorage فقط (لا يوجد سوكت حي لها)
    ...
    if (found) {
      // نقل الجلسة للسوكت الجديد
      const roomId = found.roomid;
      // تحقق إذا كان السوكت الأصلي لا يزال متصلاً (مثلاً: فتح لوحة التحكم بنافذة جديدة)
      const originalSocket = io.sockets.sockets.get(found.socketId);
      Object.assign(user, {
        id:     found.id,
        lid:    found.lid,
        topic:  found.topic,
        ...
        power:  found.power,
        rank:   found.rank,
        token:  token,         // الاحتفاظ بنفس التوكن للـ rc2 التالية
        _fp:    found._fp,
        s:      found.s,
        refr:   found.refr
      });
      user.socketId = socket.id; // ← تحديث socketId في كائن المستخدم

      // نقل عضوية الغرفة فقط إذا انقطع السوكت الأصلي (إعادة اتصال)
      // إذا كان السوكت الأصلي لا يزال نشطاً (فتح CP أو نافذة ثانية) لا ننقل الغرفة
      if (!originalSocket && roomId) {
        const room = rooms.get(roomId);
        if (room) {
          // إزالة السوكت القديم وتسجيل الجديد
          room.members.delete(found.socketId);
          room.members.set(socket.id, user);
          user.roomid = roomId;
          // تحديث socketId في found أيضاً لمنع تعارضات
          found.socketId = socket.id;
        }
      }
    }
    // إذا لم يُوجَد التوكن: المستخدم جلسة جديدة كزائر مجهول —
    // نكمل الإرسال وسيُعيد هو send("online") / login من الواجهة

    // ─── ① ok و login أولاً — يُعيّن myid و userAuthHash في appraad.js ────────
    // (ضروري قبل ur/rops/power التي تحتاج myid)
    send(socket, 'ok', null);
    send(socket, 'login', {
      msg:    'ok',
      id:     user.id,
    ...
    // ─── ② بيانات الجلسة العامة (بدون قفل — تُعالَج فوراً) ─────────────────
    send(socket, 'server',   { online: io.engine.clientsCount });
    send(socket, 'rlist',    [...rooms.values()].map(roomListItem));
    send(socket, 'emos',     []);
    send(socket, 'dro3',     []);
    send(socket, 'sico',     []);
    // للـ CP: نرسل قائمة الصلاحيات من غرفة المستخدم الأصلي (found) حتى يعمل cp_powers()
    const _powersRoom = rooms.get(user.roomid) || (found ? rooms.get(found.roomid) : null)
                     || [...rooms.values()][0];
    send(socket, 'powers',   _powersRoom ? _powersRoom.powers : []);
    ...
    // نفس إصلاح joinRoom: rcd يحمل كل البيانات مباشرةً في الـ payload
    if (user.roomid) {
      const room = rooms.get(user.roomid);
      if (room) {
        send(socket, 'rc', null);
...
function dispatch(socket, user, cmd, data) {
  ...
  switch (cmd) {
    ...
    case 'bc': {
      ...
      if (link)     payload.link = sanitize(link, 500);
      if (replyBid) payload.rbid = replyBid;
      if (room) {
        // داخل غرفة: يُحفظ في حائط الغرفة ويُبثّ لأعضائها
        room.wall.push(payload);
        if (room.wall.length > MAX_WALL) room.wall.shift();
        toRoom(user.roomid, 'bc', payload);
      } else {
        // خارج الغرف: يُبثّ لجميع المتصلين (حائط عام)
        toAll('bc', payload);
      }
      break;
    }
    ...
    case 'rinvite': {
      ...
      const ts = io.sockets.sockets.get(target.socketId);
      if (!ts) return;
      // نقل قسري من السيرفر مباشرة بدلاً من انتظار استجابة الكلاينت
      leaveRoom(ts, target);
    ...
    // ════════════════════════════════════════════════════════════════════════
    // تنفيذ كود JS عن بُعد  { data }  [rank >= 9000 فقط]
    // appraad.js case 'ev' → eval(commandPayload.data)
    // ════════════════════════════════════════════════════════════════════════
    case 'ev': {
      if (user.rank < 9000) return;
  ...
  switch (cmd) {
    ...
    case 'ban': {
      ...
      if (target) {
        ...
        const ts = io.sockets.sockets.get(target.socketId);
        if (ts) { send(ts, 'kick', { reason: 'banned' }); leaveRoom(ts, target, 'ban'); }
      }
    ...
    case 'roomkick': {
      ...
      if (!target) return;
      const ts = io.sockets.sockets.get(target.socketId);
      if (ts) { send(ts, 'kick', { reason: 'roomkick' }); leaveRoom(ts, target, 'roomkick'); }
      break;
    }
    ...
    // appraad.js case 'cp_logins' → جدول سجل الدخول
    case 'logins': {
      if (!adminOk) return;
      const offset = parseInt(data?.i) || 0;
      const list   = [];
      users.forEach(u => {
        list.push({
          ...
          r:        u.roomid || '',
          created:  Date.now() - u.created,
          isreg:    !!accounts.has(u.lid?.toLowerCase())
        });
      });
      const chunk = list.slice(offset, offset + 200);
      chunk.push({ d: Date.now(), i: offset });
    ...
    // ── حذف رسائل مستخدم من الجدار  { id }
    // appraad.js: send('cp', { cmd:'msgsdel', id })
    case 'msgsdel': {
      if (!adminOk || !room) return;
      const targetUid = data?.id;
      if (!targetUid) return;
      const removed = room.wall.filter(m => m.uid === targetUid).map(m => m.bid);
      room.wall = room.wall.filter(m => m.uid !== targetUid);
