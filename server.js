...
app.get('/cp', (req, res) => {
  ...
  // سكربت يُحقن في صفحة CP:
  // يستمع لرسالة 'con' من النافذة الأصل، وبعد 120ms يُظهر #cp ويُخفي #room
  // هذا يحل مشكلة إخفاء #cp بعد معالجة أحداث login/rcd في appraad2.js
  // السكربت المُحقن يعمل على مستويين:
 ...
 const scriptLines = [
    ...
    '  var _done=false;',
    // مرحلة capture: يعترض close المبكر ويوقف انتشاره قبل أن يصل لـ appraad2.js
    '  window.addEventListener("message",function _g(e){',
    ...
    '    }',
    '  },true);',  // true = capture phase يسبق listener في appraad2.js
    '})();'
  ];
  ...
  html = html.includes(closeTag)
    ? html.replace(closeTag, fixScript + closeTag)
    : html + fixScript;
...
// ▲ إصلاح/إضافة: case 'actions' (سجل الإجراءات الإدارية في لوحة التحكم) كان
//   مجرد stub يُرجع دائماً قائمة فارغة ([{d,i}] فقط بلا أي سجل حقيقي)، رغم
//   أن واجهة العميل (cp_actions مع دعم بحث q وترقيم صفحات i) مبنية بالكامل
//   لعرض سجل فعلي — تماماً كما هو الحال في fps/logins. أضفنا سجلاً حقيقياً
//   في الذاكرة (actionsLog) ودالة logAction() تُستخدم في كل الإجراءات
//   الإدارية الحساسة (طرد/حظر/حذف عضوية/تعديل صلاحية/كلمة مرور...).
const actionsLog = [];
const MAX_ACTIONS_LOG = 5000;
function logAction(actor, action, details = '') {
  actionsLog.push({
    t: Date.now(),
    by: actor?.topic || '', byLid: actor?.lid || '',
...
function checkFilters(text) {
  if (!text || !global.filters) return null;
  const lower = String(text).toLowerCase();
  for (const [path, v] of Object.entries(global.filters)) {
    if (v && lower.includes(String(v).toLowerCase())) return { path, v };
  }
  return null;
}
function logFilteredMessage(sender, matchedWord, fullMsg) {
...
function isMod(user, room = null) {
  ...
  if (room && user.power) {
    const def = globalPowers.find(p => p.name === user.power);
    if (def) return true;
  }
  return false;
}
...
function dispatch(socket, user, cmd, data) {
  ...
  switch (cmd) {
    ...
    case 'msg': {
      ...
      if (!text && !link) return;
      // ▲ إضافة: تطبيق فلتر الكلمات المُدار من لوحة التحكم (كان غير مُفعَّل
      //   إطلاقاً على أي رسالة سابقاً — راجع checkFilters أعلاه).
      const filterHit = !adminOk ? checkFilters(text) : null;
      if (filterHit) {
        logFilteredMessage(user, filterHit.v, text);
    ...
    case 'bc': {
      ...
      const filterHit = !isAdmin(user, room) ? checkFilters(text) : null;
      if (filterHit) {
        logFilteredMessage(user, filterHit.v, text);
...
function dispatchCP(socket, user, room, data, adminOk, modOk) {
  ...
  switch (cmd) {
    ...
    case 'kick': {
      ...
      const target = byUID(data.id);
      if (!target) return;
      logAction(user, 'kick', `طرد: ${target.topic}`);
      const ts = io.sockets.sockets.get(target.socketId);
      if (ts) {
    ...
    case 'ban': {
      ...
      logAction(user, 'ban', target ? `حظر: ${target.topic}` : `حظر بصمة: ${entry.fp}`);
    ...
    case 'aban': {
      ...
      if (!key) return;
      logAction(user, 'aban', `رفع حظر: ${key}`);
    ...
    case 'pwd': {
      ...
      const target = byUID(data.id);
      if (!target) return;
      logAction(user, 'pwd', `تغيير كلمة مرور: ${target.topic}`);
      for (const [, acc] of accounts) {
    ...
    case 'setpower': {
      ...
      if (target.power && room) {
        const def = globalPowers.find(p => p.name === target.power);
    ...
    case 'delu': {
      ...
      const target = byUID(data.id);
      if (!target) return;
      logAction(user, 'delu', `حذف عضوية: ${target.topic}`);
      // حذف من الحسابات
    ...
    case 'actions': {
      ...
      const list = actionsLog
        .filter(a => !query || a.by.includes(query) || a.action.includes(query) || a.details.includes(query))
        .slice()
    ...
    case 'roomkick': {
      ...
      const target = byUID(data.id);
      if (!target) return;
      logAction(user, 'roomkick', `طرد من الغرفة: ${target.topic}`);
      const ts = io.sockets.sockets.get(target.socketId);
      if (ts) {
    ...
    case 'delpic': {
      ...
      const target = byUID(data?.id);
      if (!target) return;
      logAction(user, 'delpic', `حذف صورة: ${target.topic}`);
      target.pic = 'pic.png';
      saveAccount(target, { pic: 'pic.png' });
      if (target.roomid) toRoom(target.roomid, 'u^', target.pub());
      break;
    ...
    case 'unban': {
      ...
      const key = data?.id || data?.type || '';
      if (!key) return;
      logAction(user, 'unban', `رفع حظر: ${key}`);
      room.bans = room.bans.map(b =>
        (b.id && b.id === key) || (b.type && b.type === key)
          ? { ...b, active: false } : b
      );
    ...
    case 'fltrdelx': {
      ...
      send(socket, 'cp_fltr', {
        a: Object.entries(global.filters || {}).map(([path, v], id) => ({ id, path, v, type: path.split('/')[0] || 'word' })),
