 ...
 *     Server → rc, ulist, rlist, emos..., power, mic, bclist, rops, rcd
...
function seedAdmin() {
  ...
  if (accounts.has(nameKey)) {
    console.log(`[Admin] الحساب موجود مسبقاً: "${adminName}"`);
    return;
  }
...
function joinRoom(socket, user, room, pwd = '') {
  ...
  // ─── ① rc أولاً — يُقفل systemCommandQueue في appraad.js ─────────────────
  send(socket, 'rc', null);

  // ─── ② بيانات الغرفة (تُخزَّن في queue ريثما يفتحها rcd) ─────────────────
  ...
  // إبلاغ بقية الأعضاء بدخول المستخدم (يُرسَل قبل rcd)
  toRoom(room.id, 'u+', user.pub(), socket.id);

  // تحديث عداد الغرفة لجميع المتصلين
  broadcastRoomUpdate(room);

  // ─── ③ rcd آخراً — يفتح queue ويُشغّل الأوامر المؤجلة ───────────────────
  // appraad.js: rcd → isSystemQueueLocked=false → تشغيل systemCommandQueue
  send(socket, 'rcd', []);

  // رسالة ترحيب اختيارية (بعد rcd حتى تُعرض في العام مباشرة)
  if (room.welcome) {
...
function leaveRoom(socket, user) {
  ...
  // appraad.js case 'u-': commandPayload = userId (string) → يزيل العنصر من الواجهة
  toRoom(room.id, 'u-', user.id);
  broadcastRoomUpdate(room);
}
...
io.on('connection', socket => {
  ...
  socket.on('rc2', ({ token, n } = {}) => {
    ...
    // ─ إرسال بيانات إعادة الاتصال بنفس ترتيب online ─
    ...
    if (user.roomid) {
      const room = rooms.get(user.roomid);
      if (room) {
        // rc يُقفل قائمة الانتظار في appraad.js — يُرسَل قبل ulist
        send(socket, 'rc',     null);
        send(socket, 'ulist',  [...room.members.values()].map(u => u.pub()));
        send(socket, 'emos',   room.emos.length ? room.emos : []);
        send(socket, 'dro3',   room.colors);
        ...
        send(socket, 'bclist', room.wall);
        send(socket, 'rops',   buildRops(room));
        send(socket, 'rcd',    []);  // ← rcd يفتح قائمة الانتظار ويُشغّل الأوامر المؤجلة
        // إبلاغ بقية الأعضاء بعودة المستخدم
        toRoom(room.id, 'u+', user.pub(), socket.id);
        broadcastRoomUpdate(room);
      }
    }

    // ok ثم login — نفس الترتيب الذي يتوقعه appraad.js في msg handler
    // appraad.js: if (cmd=='ok') isLoginApproved=true
    //             if (!isReconnecting && isLoginApproved) userSessionToken = serverResponse.k
    send(socket, 'ok', null);
    send(socket, 'login', {
      msg:    'ok',
      id:     user.id,
...
function dispatch(socket, user, cmd, data) {
  ...
  switch (cmd) {
    ...
    case 'login': {
      ...
      if (existingUser && existingUser.socketId !== socket.id) {
        ...
        if (oldSocket) {
          ...
          leaveRoom(oldSocket, existingUser);
        }
    ...
    case 'bc': {
      ...
      if (!text && !link) return;
      const newBid = makeBid();   // ID المنشور الجديد دائماً فريد
      const payload = buildMsg(user, text || '', {
        bid:  newBid,
        rbid: replyBid || null    // المردود عليه
      });
      if (link)    payload.link = sanitize(link, 500);
    ...
    case 'uh': {
      if (!adminOk) return;
      // data هو userId (string) من appraad.js
      const targetId  = typeof data === 'string' ? data : (data?.id || '');
      // ابحث أولاً بـ id للحصول على lid الحقيقي
      ...
      users.forEach(u => {
        if (u.lid === searchLid || u.id === targetId) {
          history.push({
            u:   u.topic,      // اسم الزخرفة
            t:   u.topic,      // appraad.js يقرأ logItem.t للزخرفة
            _ip: u._ip  || '',
            _fp: u._fp  || '',
            c:   Date.now() - (u.last || Date.now())
          });
        }
      });
    ...
    case 'rinvite': {
      ...
      const target = byUID(invId);
      if (!target) return;
      const ts = io.sockets.sockets.get(target.socketId);
      if (!ts) return;
      send(ts, 'rinvite', { id: rid, pwd: pwd || '' });
      break;
    }
