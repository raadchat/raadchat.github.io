...
function joinRoom(socket, user, room, pwd = '') {
  ...
  //
  // appraad.js (سطر 1727-1733):
  //   case 'rcd':
  //     isSystemQueueLocked = false;
  //     systemCommandQueue = [];   ← تُمسح القائمة قبل الاستخدام!
  //     var combinedQueueData = commandPayload.concat(systemCommandQueue);
  //                           = commandPayload + []  = commandPayload فقط
  //
  //   → الأوامر المُرسَلة بين rc و rcd تضيع تماماً
  //   → الحل: rcd نفسه يحمل [[cmdName, cmdData], ...] ليُعالجها مباشرة
  //
  // الترتيب الصحيح:
  //   rlist  → يملأ rcach (مطلوب قبل ulist و ur)
  ...
  const rcdPayload = [
    ['rlist',    [...rooms.values()].map(roomListItem)],
