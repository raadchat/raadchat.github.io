...
app.get('/', (req, res) => {
  ...
  if (contentColor) {
    ...
    // ▲ إصلاح جوهري (تأكَّد بـHTML فعلي أرسله المستخدم من التطبيق الحي):
    //   حذف .pmsgc/.ppmsgc/.hmsg من القائمة (أعلاه) لم يكن كافياً — appraad2.js
    //   يضع class="uzr d-flex mm pmsgc ..." على *نفس العنصر*: صف الرسالة
    //   يحمل .uzr (الكلاس الأساسي لكل صف رسالة/عضو) و.pmsgc معاً في آنٍ واحد.
    //   فقاعدة .uzr{...!important} هنا كانت تستمر بتلوين خلفية هذه الصفوف
    //   بالضبط كأي صف آخر، رغم عدم استهداف .pmsgc صراحة — لأن .uzr وحدها
    //   تكفي للمطابقة. .d2/#rooms/#users/#lonline/.room عناصر حاويات منفصلة
    //   لا تحمل هذه الكلاسات أبداً على نفس العنصر، فلا تحتاج نفس المعالجة.
    //   الحل: استثناء .uzr صراحة عبر :not() بدل حذفها بالكامل من القاعدة
    //   (فتبقى صفوف الأعضاء/الرسائل العادية تتبع لون المحتوى كما يجب).
    colorRules.push(`.d2,#rooms,#users,#lonline,.uzr:not(.pmsgc):not(.ppmsgc):not(.hmsg),.room{background-color:${contentColor} !important;}`);
  }
  if (buttonsColor) {
