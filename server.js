...
function dispatch(socket, user, cmd, data) {
  switch (cmd) {
    ...
    case 'setprofile': {
      if (!user.roomid) return;
      const { topic, msg: statusMsg, ucol, bg } = data || {};
      ...
      saveAccount(user, { topic: user.topic, msg: user.msg, ucol: user.ucol, bg: user.bg });
      toRoom(user.roomid, 'online+', user.pub());
      break;
    }
...
function cpDispatch(socket, user, data) {
  ...
  switch (cmd) {
    ...
    case 'domainsave': {
      ...
      send(socket, 'domainsave', { ok: true });
      break;
    }
