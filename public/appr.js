...
case "login":
  ...
  switch (commandPayload.msg) {
    ...
    case 'reg':
      ...
      $('#u2').val($("#u3").val());
      $("#pass1").val($("#pass2").val());
      login(0x2); // التحويل التلقائي لتبويب تسجيل الدخول بعد نجاح التسجيل reg
      break;
  }
  break;
case "powers":
...
function injectBroadcastItemToUi(targetContainerSelector, messagePayload) {
  ...
  if (youtubeVideoMatch.length > 0x1 && targetContainerSelector != "#d2") {
    messagePayload.msg = messagePayload.msg.replace(youtubeVideoMatch[0x1], "<button onclick='ytube(\"https://www.youtube.com/embed/" + youtubeVideoMatch[0x0] + "\",this);' style='font-size:40px!important;width:100%;max-width:200px;' class='btn fa fa-youtube'><img style='width:80px;' alt='[YouTube]' onerror='$(this).parent().remove();' src='https://img.youtube.com/vi/" + youtubeVideoMatch[0x0] + "/0.jpg' ></button>");
  }
