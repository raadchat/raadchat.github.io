...
case "cp_bots":
  ...
  $.each(commandPayload, function (index, botObj) {
    ...
    var botRowNode = appendTableRow(botsTableNode, [botObj.stat == 0x0 ? "متصل" : '', botObj.co || '--', botObj.topic, botObj.msg, formatLargeNumber(botObj.rep || 0x0) + '', anchoredRoomObj ? anchoredRoomObj.topic : '', botConfigBtnHtml], [0x8c, 0x78, 0x78, 0x78, 0x3c, 0x50]);
...
case "cp_rooms":
  ...
  $.each(commandPayload, function (index, adminRoomObj) {
    ...
    appendTableRow(roomsTableNode, [adminRoomObj.topic, adminRoomObj.user, roomSettingsBtnHtml], [0x8c, 0x78, 0x78]);
  });
...
case "cp_msgs":
  ...
  $.each(commandPayload, function (index, msgConfigObj) {
    ...
    appendTableRow(msgsTableNode, [msgConfigObj.type == 'w' ? "الترحيب" : "الرسائل", msgConfigObj.t, msgConfigObj.m, deleteMsgBtnHtml], [0x5a, 0x8c, 0x118, 0x50]);
  });
...
case "cp_shrt":
  ...
  $.each(commandPayload, function (index, shortcutObj) {
    ...
    appendTableRow(shortcutsTableNode, [shortcutObj.name, shortcutObj.value, deleteShortcutBtnHtml], [0x50, 0x190, 0x50]);
  });
...
case "cp_fltr":
  ...
  $.each(commandPayload.a, function (index, filterObj) {
    ...
    appendTableRow(filterTableNode, [filterObj.type, filterObj.v, deleteFilterBtnHtml], [0x5a, 0x12c, 0x50]);
  });
...
case "cp_bans":
  ...
  $.each(commandPayload, function (index, banItemObj) {
    ...
    appendTableRow(bansTableNode, [banItemObj.user, banItemObj.type, banItemObj.date, banItemObj.co, banItemObj.lc, banActionsBtnHtml], [0x50, 0xbe, 0x78, 0x54]);
  });
...
$(".ae").click(function (event) {
  setTimeout(function () {
    ...
    setTimeout(function () {
      switchLoginTabs(actionTypeCode);
    }, 320);
    return;
  }
...
}).text(formatLargeNumber(targetUserObj.rep || 0x0) + '');
...
function openPrivateChatWindow(targetUserIdParam, shouldFocusWindow) {
  ...
  if ($('#c' + targetUserIdParam).length == 0x0) {
    ...
    $(privateChatWindowNode).find(".fa-share-alt").click(function () {
      uploadAndShareMedia(targetUserIdParam);
    });
...
window.roomspic         = uploadAndSetImageSrc;   // رفع وتغيير صورة الغرفة
...
window.cp_powers        = filterAndRenderPowersList;
window.cp_bots          = openBotManagementMenu;
window.cp_powerchange   = filterAndRenderPowersList;
window.sett_save        = saveSiteSettings;
window.domains_save     = saveDomainSettings;
window.emo_order        = reorderEmojis;
window.del_ico          = deleteSupervisorIcon;
window.sendfilea        = uploadAndShareMedia;
window.cp_fps           = openFpsSearch;
window.cp_fps_do        = openBanSearchMenu;
window.cp_ledit         = openUserManagementMenu;
window.uprochange       = filterAvailablePowersUi;
window.s_sico           = addSupervisorIcon;
window.redit            = openEditRoomModal;         // فتح لوحة إداره وتعديل وروم كنترول الغرفة
window.fltrit           = filterTableData;
  ...
  openContextMenu(_0x2179bf, "الزخرفه,الوصف,الدوله,اللون,لون الخلفيه,تسجيل دخول,تسجيل خروج,الصوره,حذف الصوره,الغرفه,----,حذف".split(','), function (selectedOption) {
    switch (selectedOption) {
      ...
      case "الدوله":
        ...
        if (typeof countryCode == "string" && countryCode.length == 0x2 && uf[countryCode.toLowerCase()] != null) {
          send('cp', {
            'cmd': "bot",
            'id': _0x2a7140,
